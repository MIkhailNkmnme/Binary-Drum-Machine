...
var P = {
  ...
  trackLine:false, trackName:'', title4:'', trkSize:60, trkFont:0, trkSpace:0,   // строка 4 — название трека (только на сайте)
  ...
  rainOwn:false, rainColor:'#00ff6e',   // свой цвет дождя (только на сайте)
  // холм
...
var ZERK_VERSION='1.9';   // версия обоев
...
// размер сцены: весь экран или вертикальная рамка «телефон 9:16» (её задаёт zvuk.js)
function stageW(){ return window.ZERK_PHONE ? Math.max(90,Math.round(innerHeight*9/16/2)*2) : innerWidth; }
function stageH(){ return innerHeight; }
var camera=new THREE.PerspectiveCamera(50,stageW()/stageH(),1,6000);
...
function vParams(){
  var W=stageW(),H=stageH(), cx=W/2, cy=H*(P.vCY/100);
...
// геометрию освобождаем: иначе каждая пересборка оставляет прежнюю сетку в видеопамяти
function clearScene(){
  var seen=[];
  objs.forEach(function(o){ scene.remove(o); if(o.geometry && seen.indexOf(o.geometry)<0){ seen.push(o.geometry); o.geometry.dispose(); } });
  objs=[];
}
// пересборка сцены после ползунка — когда его отпустили, а не на каждом шаге
var rebuildT=0;
function rebuildLater(sc,fn){ clearTimeout(rebuildT); rebuildT=setTimeout(function(){ if(P.scene===sc) fn(); },150); }
var HILL_MAX=120000;   // потолок символов холма: при «пыли» сетка 500×500 душила страницу
...
function buildHill(){
  ...
  var kk=Math.sqrt(HILL_MAX/((segX+1)*(segZ+1)));
  if(kk<1){ segX=Math.max(12,Math.floor(segX*kk)); segZ=Math.max(12,Math.floor(segZ*kk)); }
...
// layout — всё целиком (смена размера окна); отдельно дождь и надпись: правка надписи не стирает дождь
function layout(){
...
function layoutTitle(){
  ...
  var src=[[P.title,P.t1Size,P.t1Font,P.t1Space],[P.title2,P.t2Size,P.t2Font,P.t2Space],[P.title3,P.t3Size,P.t3Font,P.t3Space],
           [P.trackLine?(String(P.title4||'').trim()||P.trackName):'',P.trkSize,P.trkFont,P.trkSpace]];
  ...
  // все строки пустые — надписи нет; «Zerkalius» остаётся только меркой для размера и места часов
  var noText=!L.length;
  if(noText) L=[{text:'Zerkalius',k:1,fc:fontCss(0),sp:0}];
  ...
  var fsBase=Math.min(probe*0.62*W/wmax, H*(nl===1?0.17:nl===2?0.13:nl===3?0.10:0.085)/0.63)*(P.titleSize/100);
  ...
  L.forEach(function(l){
    ...
    if(!noText) mx.fillText(l.text,W/2+l.ls/2,l.base);        // +ls/2: у последней буквы хвост интервала
    tw=Math.max(tw,mx.measureText(l.text).width); fsMax=Math.max(fsMax,l.fs); fsMin=Math.min(fsMin,l.fs);
  });
  mx.letterSpacing='0px';
  var fs=fsMax;
  T={lines:noText?[]:L,fs:fs,W:W,H:H,cy:cy,top:top,bot:top+total,w:tw,ds:Math.max(9,Math.round(fsMin/19)),cells:[]};
  var cw=Math.round(T.ds*0.56)+1, ch=Math.round(T.ds*0.9);
  var x0=W/2-tw/2-10, x1=W/2+tw/2+10;
  // читаем с маски только полосу надписи, а не весь экран — так правка надписи на «авто» не тормозит
  var gx0=Math.max(0,Math.floor(x0)), gy0=Math.max(0,Math.floor(T.top-T.ds)),
      gx1=Math.min(W,Math.ceil(x1+cw)), gy1=Math.min(H,Math.ceil(T.bot+fs*0.25+ch)), gw=gx1-gx0;
  if(!noText && gw>0 && gy1>gy0){
    var md=mx.getImageData(gx0,gy0,gw,gy1-gy0).data;
    for(var y=T.top;y<T.bot+fs*0.25;y+=ch) for(var x=x0;x<x1;x+=cw){
      var qx=Math.round(x+cw/2), qy=Math.round(y-T.ds*0.35);
      if(qx>=gx0&&qx<gx1&&qy>=gy0&&qy<gy1&&md[((qy-gy0)*gw+(qx-gx0))*4+3]>60) T.cells.push([x,y,rch(),Math.random()]);
    }
  }
  ...
  T.lines.forEach(function(l){ hx.font=l.font; hx.letterSpacing=l.ls+'px'; hx.lineWidth=l.fs*0.35;
...
function loop(){
  ...
  if(acc>=1/20){ acc=0; drawRain(P.rainOwn?hexToRgb(P.rainColor):c); drawTitle(c,tt);
...
// название трека для строки 4 — его передаёт zvuk.js (имя файла или вкладки); написанное в поле строки 4 главнее
window.zerkSetTrack=function(name){ P.trackName=name||''; if(P.trackLine) layoutTitle(); };
...
function applyValue(name,val){
  switch(name){
    ...
    case 'title':  P.title=val; layoutTitle(); break;
    case 'title2': P.title2=val; layoutTitle(); break;
    case 'title3': P.title3=val; layoutTitle(); break;
    case 't1Size': case 't2Size': case 't3Size': case 't1Font': case 't2Font': case 't3Font':
    case 't1Space': case 't2Space': case 't3Space': P[name]=val; layoutTitle(); break;
    case 'trackLine': case 'title4': case 'trkSize': case 'trkFont': case 'trkSpace': P[name]=val; layoutTitle(); break;
    case 'clockFont': case 'clockSpace': P[name]=val; layoutTitle(); break;
    case 'chars':  P.chars=val; setChars(val); buildAtlas(); layoutTitle(); break;
    ...
    case 'blockY': P.blockY=val; layoutTitle(); break;
    case 'gapTC': P.gapTC=val; layoutTitle(); break;
    case 'titleSize': P.titleSize=Math.max(10,val); layoutTitle(); break;
    case 'clockSize': P.clockSize=Math.max(10,val); layoutTitle(); break;
    ...
    case 'rainSize': P.rainSize=Math.max(3,val); layoutRain(); break;
    case 'rainTrail': P.rainTrail=val; break;
    case 'rainOwn': P.rainOwn=val; break;
    case 'rainColor': P.rainColor=val; break;
    case 'rainGapX': P.rainGapX=Math.max(1,val); layoutRain(); break;
    case 'rainGapY': P.rainGapY=Math.max(1,val); layoutRain(); break;
    // холм
    case 'hBright': P.hBright=val; break;
    case 'hBig':    P.hBig=Math.max(0.2,val/10); rebuildLater(0,buildHill); break;
    case 'hSpeed':  P.hSpeed=val; break;
    case 'hCover':  P.hCover=val; break;
    case 'hGapX':   P.hGapX=Math.max(2,val); rebuildLater(0,buildHill); break;
    case 'hGapY':   P.hGapY=Math.max(2,val); rebuildLater(0,buildHill); break;
    // радар
    ...
    case 'rDense':  P.rDense=val; rebuildLater(2,buildRadar); break;
    // воронка
    ...
    case 'sWidth':  P.sWidth=val; rebuildLater(1,buildSierp); break;
    case 'sDepth':  P.sDepth=val; rebuildLater(1,buildSierp); break;
  }
}
...
function livelyPropertyListener(name,val){
  ...
  if(name.indexOf('auto_')===0){                      // галочка «авто» у ползунка
    var n=name.slice(5); P.auto[n]=!!val; delete AUTOLAST[n];
...
// «авто» у любого ползунка. Тяжёлые шевелятся реже: пересборка сетки — раз в секунду,
// пересчёт надписи/дождя/воронки — раз в четверть секунды; значение применяется, только если сменилось.
var AUTO_SLOW={hBig:1,hGapX:1,hGapY:1,rDense:1,sWidth:1,sDepth:1};
var AUTO_MID={t1Size:1,t2Size:1,t3Size:1,t1Space:1,t2Space:1,t3Space:1,trkSize:1,trkSpace:1,titleSize:1,blockY:1,gapTC:1,
              clockSize:1,clockSpace:1,rainSize:1,rainGapX:1,rainGapY:1,vCY:1,vHole:1,vSize:1,vStep:1,vGap:1,vTwist:1,rCY:1,rSpan:1};
var AUTOLAST={};
function autoTick(t){
  var amp=P.autoAmp/100; if(amp<=0) return;
  for(var n in P.auto){
    if(!P.auto[n] || n==='autoAmp') continue;
    var p=(window.LIVELY_PROPS||{})[n]; if(!p||p.type!=='slider') continue;
    var L=AUTOLAST[n]||(AUTOLAST[n]={t:-9,v:null});
    var gap=AUTO_SLOW[n]?1:AUTO_MID[n]?0.25:0;
    if(t-L.t<gap) continue;
    var b=baseOf(n);
