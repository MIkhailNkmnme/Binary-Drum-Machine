...
function makeColResizer(el, varName, selectorOfCol, minPx, onManual, invert){
  ...
  el.addEventListener("mousedown", e => {
    ...
    // ЗЕРКАЛЬНЫЙ ЗНАК (v0.986) — для границы на ВНЕШНЕМ (левом) крае П1: та растёт, когда тянут
    // ВЛЕВО, а не вправо, как остальные три. Один и тот же обработчик на всех, эта строка — вся
    // разница между ними.
...
makeColResizer(document.getElementById("vsplitL0"), "--pat-w", ".pat", 40, () => { patWManual = true; }, true);
...
}
{
  ...
  if (vL0) vL0.addEventListener("dblclick", e => { e.preventDefault(); fitPatColumnTo("--pat-w"); });
...
{
  const rowsPanEl = document.getElementById("rows");
  ...
  if (rowsPanEl) rowsPanEl.addEventListener("mousedown", e => {
    ...
    const ln = e.target.closest(".ln");
    /* КРОМЕ .ln — ЕЩЁ И ПУСТОЕ МЕСТО ВНУТРИ #rows (v1.006, баг-репорт пользователя: "биты стоят
       на своих строках, но все строки сдвинуты вниз за нижнюю границу поля — биты центрального
       поля не захватываются"). При большом bitsOffY глифы .bits>span (position:relative + top)
       визуально уезжают НИЖЕ СВОЕЙ ЖЕ строки — и если это последние строки списка, уезжают за
       пределы ПОСЛЕДНЕГО .ln вообще, в пустоту #rows под ним (спейсер виртуализации/просто край).
       e.target.closest(".ln") там честно null, и раньше жест тут же обрывался. Теперь достаточно
       попасть КУДА УГОДНО внутрь #rows — какую именно строку хватать, решает fieldAtEventEl() её
       собственным перебором (см. её код: если под конкретной .ln бит не нашлось, она проверяет
       ВСЕ отрисованные строки по их реальной geometry, а не только ту, что под курсором). */
    if (!ln && !(e.target.closest && e.target.closest("#rows"))) return;
