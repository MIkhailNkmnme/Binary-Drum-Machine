...
document.addEventListener("keydown", e => {
  ...
  if (e.key === "Enter") {
    ...
    if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
      /* vsplitL0 (v1.010) — эта граница больше НЕ меняет ширину П1 (та осталась только у vsplit),
         а двигает ВСЕ ТРИ поля разом, тем же шагом-столбцом, что и её мышиная протяжка (см. блок
         "vsplitL0 — СДВИГ ВСЕХ ТРЁХ ПОЛЕЙ" в fold-5-ui.js) — со своим стопором влево до нуля. */
      if (activeBorderId === "vsplitL0") {
        const step = e.key === "ArrowRight" ? 1 : -1;
        let d = step;
        const floorD = -Math.min(patOffL || 0, st.axisCenterOffset || 0, patOffR || 0);
