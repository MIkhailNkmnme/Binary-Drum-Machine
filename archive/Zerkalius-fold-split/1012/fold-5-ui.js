...
/* ═══ vsplitL0 — СДВИГ ВСЕЙ РАСКЛАДКИ (v1.011, запрос пользователя: "левая граница сейчас
   ...
   .vsplit*/#colHeader/#axisSplit — все position:absolute ОТНОСИТЕЛЬНО .chain, поэтому едут вместе
   с ней бесплатно, никакого пересчёта their left не нужно. Слева проступает родной фон .canvas
...
const PIN_SLOTS_N = 3;   // столько пустых ячеек с каждой стороны — если мало/много, менять только тут
let pinSlots = { L: new Array(PIN_SLOTS_N).fill(null), R: new Array(PIN_SLOTS_N).fill(null) };
function renderPinSlot(container, side, idx){
  ...
  btn.addEventListener("click", e => { e.stopPropagation(); src.click(); });
...
const DEFAULT_UI_SETTINGS = {
  ...
  patOffLY: 0, patOffRY: 0, bitsOffY: 0, chainShiftCols: 0, layerFocus: true, msgPos: "", menuBarBottom: false,
  axisSnapCols: [],
  axisSnapGroups: [],
