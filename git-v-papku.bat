...
if exist ".git\" (
  echo История уже лежит внутри этой папки - переносить нечего.
  echo.
  echo === Где что лежит ===
  echo.
  echo Рабочая папка:
  echo    %CD%
  echo Каталог истории:
  for /f "delims=" %%p in ('git rev-parse --absolute-git-dir 2^>nul') do echo    %%p
  echo.
  echo Откуда тянется обновление:
  git remote -v
  if exist ".git\objects\info\alternates" (
    echo.
    echo Часть объектов берётся из чужого хранилища:
    type ".git\objects\info\alternates"
    echo Поэтому ту папку удалять нельзя - без неё история развалится.
  )
  echo.
  echo Если git ругался на другой путь, значит обновление запускали
  echo из другой папки. Покажите мне этот вывод целиком.
  goto :konec
)
