...
function pasteBitsHtml(txt){
  ...
  for (let i = 0; i < s.length; ) {
    ...
    out += (ch === "1" || ch === "0") ? '<i class="ps' + ch + '">' + run + "</i>" : esc(run);
    i = j;
  }
  return out;
}
  ...
  for (let i = vr.lo; i <= vr.hi; i++){
    ...
    if (st.paste && st.paste.rows && st.paste.row >= 0) {
      ...
      if (prow && prow.txt) {
        ...
        const pasteCols = st.paste.col + (prow.off || 0) + (st.axisCenterOffset || 0) + (prow.hf || 0) / 2 + (st.paste.half || 0) / 2;
        const leftPx = (pasteCols * colStepPx).toFixed(2);
        const offCls = (st.paste.on === false ? " paste-off" : "");
        insHtml = '<span class="paste-bits' + offCls +
          ...
          ' Тяни мышью — переставить ВЕСЬ блок по столбцам и строкам">' + pasteBitsHtml(prow.txt) + "</span>";
        /* РУЧКА — ОДНА НА БЛОК, ПРЯМО НА ЕГО ПЕРВОЙ ЦИФРЕ (v1.109, запрос пользователя: "ручки нет,
           поставь её на первую цифру блока, и ещё какое-то увеличение первой строки в других битах").
           В v1.107—1.108 ручка стояла СЛЕВА от блока и печаталась знаком «✥»: в моноширинном шрифте
           цепочки этого глифа нет, и вместо ручки на строке вылезал широкий «тофу»-прямоугольник —
           то самое «увеличение первой строки». Теперь ручка — не текст вовсе, а пустая рамка в один
           столбец шириной и во всю высоту строки, положенная поверх ПЕРВОЙ цифры блока: попасть в
           неё мышью легко, цифра под полупрозрачной заливкой видна, а печатать нечего — значит и
           ломать строку нечем.
           Строка выбирается ПЕРВАЯ НЕПУСТАЯ (пустые внутри куска сохраняются — ими держится
           вертикальный интервал, и на такой строке цифры, куда её класть, просто нет).
           Тянется тем же обработчиком, что и сами цифры (см. mousedown в fold-4-tools.js), и двигает
           ТОЛЬКО наложение: биты цепочки под ним остаются на месте. */
        let gripK = 0;
        while (gripK < st.paste.rows.length && !(st.paste.rows[gripK] && st.paste.rows[gripK].txt)) gripK++;
        if (pk === gripK) {
          insHtml += '<span class="paste-grip' + offCls + '" style="left:' + leftPx +
            'px;width:' + colStepPx.toFixed(2) +
            'px" title="Ручка наложения — тяни, и ВЕСЬ вставленный блок едет по столбцам и строкам, отдельно от бит цепочки под ним. То же делает протяжка за сами цифры блока"></span>';
        }
      }
    }
