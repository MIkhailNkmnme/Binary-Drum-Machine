...
document.addEventListener('mousedown', (e) => {
  if (e.button === 1) { e.preventDefault(); toggleAllPins(); }
});
...
if (colHeaderEl) {
  // ВЫДЕЛЕНИЕ СТОЛБЦА ЛИНЕЙКОЙ УБРАНО (запрос пользователя: "убери активацию столбцов по этим
  // элементам — это от старого кода осталось"). Столбец выбирается кликом ПО САМОМУ БИТУ при
  // включённом "🔢 Выбор столбца" (см. colPickMode в fold-4) — линейка для этого больше не нужна,
  // как и подсветка столбца по наведению на её ячейку. Клик по линейке остался только у двух
  // ЯВНЫХ режимов-кнопок, у которых другого способа указать столбец нет: "обрезка" и "⊙ Ось сюда"
  // (см. setColClickMode) — в обычном режиме ("сдвиг к «1»") линейка не делает ничего.
  colHeaderEl.addEventListener("click", e => {
    ...
    // "⇅ Сдвиг к «1»" — это НЕ выделение столбца, а сама операция (крутит строки так, чтобы в
    // этом столбце встала «1»), и другого способа указать ей столбец нет — она остаётся. А вот
    // прежние "клик = выделить столбец" и "клик по выделенному = снять" убраны.
    if (colClickMode === "shift") selectColumn(col);
  });
}
