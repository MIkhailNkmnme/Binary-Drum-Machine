...
/* Скрыть/показать колонки паттернов по отдельности — прячет .pat/.pat2 и их ручки
   ...
   Подсветка mode-act — ПОКА ПАНЕЛЬ ВИДНА (!hidden), а не пока скрыта — запрос пользователя
...
if (bTogglePatLEl) {
  bTogglePatLEl.onclick = () => {
    ...
    bTogglePatLEl.classList.toggle("overlay-on", !hidden);
    saveCache();
  };
}
...
if (bTogglePatREl) {
  bTogglePatREl.onclick = () => {
    ...
    bTogglePatREl.classList.toggle("overlay-on", !hidden);
    saveCache();
  };
}
