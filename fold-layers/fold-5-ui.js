...
function updateSplitPositions(){
  ...
  const placeStrip = (stripId, edgeEl, side) => {
    ...
    const wantLeftStrip = (side === "L")
      ? (box.right - cr.left - (strip.offsetWidth || 60) - 2)
      : (box.right - cr.left + 4);
    strip.style.left = Math.max(0, wantLeftStrip) + "px";
  };
...
function updateAxisSplitPosition(maxLen){
  ...
  axisSplitEl.style.height = Math.round(axisLineH) + "px";
     ...
     top — сразу под ручкой оси, той же формулой, что и её высота. */
  {
    ...
    if (strip) {
      ...
      const patForStrip = document.querySelector("#colHeader .pat") || document.querySelector(".ln .pat");
      const stripLeft = (patForStrip && !document.body.classList.contains("hide-pat-l"))
        ? (patForStrip.getBoundingClientRect().right - chainRect.left + 1)
  ...
  if (alignGrpEl && mainEl) {
    ...
    const numStripW = (() => {
      if (!numStripEl || !numStripEl.classList.contains("act") || !bitsBox) return 0;
      const sr = numStripEl.getBoundingClientRect();
      return Math.max(0, sr.right - bitsBox.left) + 8;
    })();
