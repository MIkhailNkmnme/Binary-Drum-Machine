...
function cycleRowNumMode(){
  const n = ROW_NUM_ORDER.length;
  let i = ROW_NUM_ORDER.indexOf(st.rowNumMode || "dec");
  if (i < 0) { i = 0; rowNumDir = 1; }
  let next = i + rowNumDir;
  if (next < 0 || next >= n) { rowNumDir = -rowNumDir; next = i + rowNumDir; }
  st.rowNumMode = ROW_NUM_ORDER[next];
  applyRowNumMode();
...
function makeColResizer(el, varName, selectorOfCol, minPx, onManual, invert, fieldKey){
  ...
  el.addEventListener("mousedown", e => {
    ...
    const move = ev => {
      ...
      const w = Math.max(minNow, Math.round(startW + sign * (ev.clientX - startX)));
      document.documentElement.style.setProperty(varName, w + "px");
...
makeColResizer(document.getElementById("vsplit"), "--pat-w", ".pat", 40, () => {
  patWManual = true;
  /* Ширину только что выставили глазами — значит она УЖЕ такая, какой человек её видит, вместе с
     номером, если тот показан (v1.303). Переставляем флаг прибавки под текущее состояние кнопки,
     иначе fitPatW() либо прибавит номер второй раз, либо вычтет никогда не добавленное. */
  patWNumReserved = !!st.patNumL;
  patWNumReservedPx = patWNumReserved
...
function minBitsWidthPx(){
  ...
  const el = document.getElementById("axisStrip");
  const numW = (el && el.classList.contains("act")) ? el.offsetWidth + 4 : 0;
  return Math.round(g.offsetWidth + numW + 24);
}
...
function updateSplitPositions(){
  ...
  {
    const dotIns = row.querySelector(".pat-dot");
    const numIns = row.querySelector(".num-l2");
    let ins = 0;
    if (dotIns && numIns) {
      const nb = numIns.getBoundingClientRect();
      if (nb.width > 0) ins = nb.width + (nb.left - dotIns.getBoundingClientRect().right);
    }
    /* ═══ НОМЕР ВНУТРИ П1 ВСТАВЛЯЕТСЯ ТОЧНО ТАК ЖЕ (v1.319) ═══
       Запрос пользователя: «пусть номера строк у П1 поля тоже все слева сдвигают для себя».
       Номер П1 (.num-p1) живёт ВНУТРИ ячейки паттерна, сразу за его текстом, и место под себя
       берёт не сам — его резервирует КОЛОНКА: fitPatW() прибавляет к --pat-w ровно --num-w, когда
       включена body.patnum-l (в ручном режиме — та же прибавка разово, см. patWNumReserved там
       же). Колонка П1 стоит первой в строке, поэтому её прибавка толкала вправо и цепочку.
       Прибавляем ту же величину к общей вставке — и цепочка остаётся на месте, а влево уезжает
       текст паттерна вместе с левым краем колонки. Номер занимает открывшееся место у её правого
       края, где он и печатается.
       БЕРЁМ CSS-ПЕРЕМЕННУЮ, А НЕ КОРОБКУ ЭЛЕМЕНТА. Это единственный случай, где мерить нельзя:
       раскладку двигает не глиф, а РЕЗЕРВ колонки, и он один на все строки (--num-w — самый
       широкий номер), тогда как сам .num-p1 в каждой строке своей длины («1» против «1011»).
       Мерь мы элемент — сдвиг гулял бы от строки к строке. Читаем ровно ту же переменную и по
       тому же условию, что и fitPatW(), иначе прибавка и вычет разойдутся.
       П2 сюда не входит и не должна: её колонка лежит СПРАВА от цепочки и подвинуть та ничего не
       может — номер там растёт в свою сторону. */
    if (st.patNumL) {
      ins += parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--num-w")) || 0;
    }
    document.documentElement.style.setProperty("--rownum-ins", Math.max(0, ins).toFixed(2) + "px");
  }
  ...
  const alignBarTop = (hLineForStrips && hLineForStrips.classList.contains("act"))
    ? ((parseFloat(hLineForStrips.style.top) || 0) + 3) : null;
  const placeStrip = (stripId, edgeEl, side, centerX) => {
    ...
    const wantLeftStrip = (side === "L")
      ? (anchorX - (strip.offsetWidth || 60) + labW / 2)
      : (anchorX - labW / 2);
    ...
    const minLeftStrip = -cr.left + 2;
    strip.style.left = Math.max(minLeftStrip, wantLeftStrip) + "px";
  };
  ...
  const patLineX = patEl ? (patEl.getBoundingClientRect().right - cr.left + 2) : 0;
  placeStrip("patStripL", patLHidden ? null : patEl, "L",
             patLHidden ? undefined : (patLineX + labLW / 2));
  // Правый просвет ни с кем не делится — кнопка «№» живёт только у левой границы.
  placeStrip("patStripR", bitsEl, "R",
             patRHidden ? undefined : gutterCenter(bitsEl, pat2El));
...
function fitPatW(){
  ...
  if (patWManual) {
    const root = document.documentElement;
    const cur = parseFloat(getComputedStyle(root).getPropertyValue("--pat-w")) || 0;
    const wantReserve = numW > 0;
    if (wantReserve && !patWNumReserved) {
      root.style.setProperty("--pat-w", Math.round(cur + numW) + "px");
      patWNumReserved = true;
    } else if (!wantReserve && patWNumReserved) {
      root.style.setProperty("--pat-w", Math.max(40, Math.round(cur - patWNumReservedPx)) + "px");
      patWNumReserved = false;
    }
    if (wantReserve) patWNumReservedPx = numW;
    return;
  }
...
function updateTopHorizon(){
  ...
  el.style.top = (rowsEl.getBoundingClientRect().top - chainEl.getBoundingClientRect().top
                  + h * rowH) + "px";
...
function positionStairsAxisBoxes(axisLeftPx, axisTopPx, axisH){
  ...
  const mid = axisTopPx + axisH / 2;
  box.style.top = Math.round(mid - box.offsetHeight / 2) + "px";
  // Левая прижата правым краем, правая — левым: обе смотрят на ось, между ними её линия.
  const left = (box === gL) ? (axisLeftPx - 6 - box.offsetWidth) : (axisLeftPx + 8);
...
function updateAxisSplitPosition(maxLen){
  ...
  const axisGapPx = (() => {
    const v = parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--axis-gap-h"));
    return isFinite(v) && v > 0 ? v : axisLinePitch * 2;
  })();
  ...
  const axisLineH = Math.max(14, Math.min(Math.max(16, rowsTop), axisGapPx - 6));
  ...
  const axisTopPx = Math.round(Math.max(0, axisLineBottom - axisLineH));
  const axisLineHFull = axisLineH + axisLinePitch;
  ...
  positionStairsAxisBoxes(leftPx, axisTopPx, axisLineH);
     ...
     top — сразу под ручкой оси, той же формулой, что и её высота. */
  {
    ...
    if (strip) {
      ...
      const fieldLeftPx = bitsRowEl
        ? (bitsRowEl.getBoundingClientRect().left - chainRect.left)
        : (bitsRect.left - chainRect.left);
      const stripWNum = strip.offsetWidth || 60;
      const btnTailNum = numBtn ? Math.max(0, stripWNum - (numBtn.offsetLeft + numBtn.offsetWidth)) : 0;
      strip.style.left = Math.round(fieldLeftPx + btnTailNum - stripWNum) + "px";
      ...
      const alignBarTop2 = (hLineForStrip2 && hLineForStrip2.classList.contains("act"))
        ? ((parseFloat(hLineForStrip2.style.top) || 0) + 3) : null;
  ...
  if (alignGrpEl && mainEl) {
    ...
    const wantLeft = bitsBox
      ? (bitsBox.left - mainRect.left) + numStripW + (bitsBox.width - numStripW - alignGrpEl.offsetWidth) / 2
      : (mainRect.width - alignGrpEl.offsetWidth) / 2;
