...
const ROW_NUM_NOTE = {
  ...
  bin: "Номера строк в колонке поля — двоичные. Номера внутри паттернов это не трогает: у них своя кнопка «🔢 Двоичные №» во вкладке «Вид».",
...
function updateSplitPositions(){
  ...
  if (v3 && pat2El && bitsEl && !document.body.classList.contains("hide-pat-r")) {
    ...
    /* ═══ П2 НЕ ПОЛЗЁТ ОТ ПРОХОДА К ПРОХОДУ (испр. v1.605) ═══
       Баг-репорт: «пусть когда паттерн выделяется и находится — не двигается ничего».
       .bits растягивается (flex-grow) на всё, что оставила П2, и кончается там, где «конец строки
       минус нынешняя П2». А строка (.ln во всю .chain) кончается на несколько пикселей раньше края
       окна (6px в замере). Ширина П2 «до края окна от края бит» выходила на каждый проход раскладки
       на эти пиксели больше: выделение паттерна давало три прохода — П2 134 → 140 → 146, биты и ось
       ехали, а fitPatW2 при снятии выделения сбрасывал обратно.
       Дальше конца строки П2 всё равно не заходит — там её правый край при любой ширине. Поэтому
       кромку берём не дальше конца строки: пока строка короче окна, расчёт отдаёт ровно ту ширину,
       что уже стоит, и сколько бы проходов ни было, ничего не едет. Строка длиннее окна (биты за
       кромкой) — как раньше, П2 сжимается до 40.
       (Попытка мерить от базовой ширины бит --bits-w не годится: та сама зависит от полосы меню 2,
       а полоса — от поля бит, и петля выходит новая.) */
...
const TIPS = {
  ...
  t24: "Снять жёлтое выделение столбца. Синий назначенный столбец («⊙ Ось сюда») это не трогает — его снимает соседняя кнопка «✕ Ось»",
  ...
  t37: "Условие для РУЧНОГО нажатия «⬆ Вверх»: «только при находке» — кнопка сработает лишь тогда, когда фон-поиск прямо сейчас нашёл искомый паттерн, а без находки просто скажет «стоп» и ничего не построит. «Всегда» — обычное поведение, строит по нажатию в любом случае. На автодостройку по находке эта настройка не влияет",
  t38: "Достраивать вверх САМО на каждой находке — то же самое, что нажать «⬆ Вверх», в текущем режиме (переписывать/дописывать). Работает везде, где находка вообще случается: прогон «🚀 Авто», ручной круговой сдвиг ◄/►, шаг и прогон «🧩 Цепочка»",
  ...
  t40: "Выключить СРАЗУ ВСЁ в этой панели — «🖱 Выдел», «✋ Вручную: только при находке», «🎯 При находке», «◀ Зеркало», «▶ Зеркало» — и заодно убрать все достроенные сверху строки. Одно нажатие возвращает цепочку к обычному виду",
  ...
  t43: "Режим выбора ЯЧЕЕК: щелчок по биту и протяжка ДОБАВЛЯЮТ к выбору (прежнее не снимается), Ctrl+щелчок или Ctrl+протяжка с выбранного бита — снять, Shift+щелчок — прямоугольник от последнего выбранного бита, Esc — снять всё разом. Выбор по порядку нажатий виден в «🧾 Черновике»; «📍 Закрепить» откладывает его группой, которую Esc не снимает. Пока режим включён, клик по строке её не выделяет. Кнопки ниже работают ровно по выбранным ячейкам",
  ...
  t88: "Сквозная змейка, начиная СЛЕВА: все строки от 1-й до выделенной склеены по порядку, но у КАЖДОЙ ВТОРОЙ строки биты реверснуты — первая строка читается слева направо, вторая справа налево, третья снова слева. Зеркальная пара — «ЗмейПрав», она начинает с правого края",
  t89: "Сквозная змейка, начиная СПРАВА: первая строка читается справа налево, вторая — слева направо, третья снова справа, и так далее. Зеркальная пара к «ЗмейЛев», где первая строка читается слева",
...
/* ═══ «🔎 КНОПКИ» — ПОИСК КНОПОК ПО ВСЕМ ПАНЕЛЯМ (v1.631) ═══
   Запрос пользователя: «окно для поиска кнопок» (после того как панели ужали до коротких подписей).
   Список собирается заново на каждый ввод: подписи у переключателей меняются на ходу, а панели
   ездят между доком, выпадающим меню, холстом и отдельным окном. Берутся все кнопки и галки
   (label.chk с флажком) из панелей MENUS, кроме скрытых дублей #retiredBtns и самого этого окна.
   Ищется по подписи, подсказке (title или TIPS[data-tip]) и имени панели, без учёта регистра и ё;
   все слова запроса должны найтись, совпавшие в подписи — выше. Показать = раскрыть панель (как
   щелчок по вкладке: в доке и на холсте она и так видна, в отдельном окне — поднять его, иначе —
   открыть выпадающее меню), прокрутить к кнопке и мигнуть рамкой. Нажать = то же и click(). */
(function btnFindInit(){
  const inp = document.getElementById("btnFindInput"), out = document.getElementById("btnFindList");
  if (!inp || !out || typeof MENUS === "undefined") return;
  const norm = s => String(s || "").toLowerCase().replace(/ё/g, "е").replace(/\s+/g, " ").trim();
  let found = [];
  function collect(){
    const items = [];
    for (const mid in MENUS) {
      if (mid === "menuFind") continue;
      const m = MENUS[mid]; if (!m || !m.wrapper) continue;
      const pan = String(m.title || mid).replace(/\s+/g, " ");
      m.wrapper.querySelectorAll("button, label.chk").forEach(el => {
        if (el.closest("#retiredBtns")) return;
        if (el.tagName === "LABEL" && !el.querySelector('input[type="checkbox"]')) return;
        const lab = (el.textContent || "").replace(/\s+/g, " ").trim();
        const key = el.dataset && el.dataset.tip;
        const tip = (key && typeof TIPS !== "undefined" && TIPS[key]) || el.title || "";
        if (!lab && !tip) return;
        items.push({ el, mid, pan, lab: lab || "(" + tip.slice(0, 20) + ")", tip, nLab: norm(lab), nAll: norm(lab + " " + tip + " " + pan) });
      });
    }
    // v1.632: и подвал списка цепочек (▾ у имени цепочки) — Экспорт/Импорт, Excel, кэш паттернов, ◇ Ромбы
    const dd = document.getElementById("chainDdList");
    if (dd) dd.querySelectorAll(".chain-dd-footer .chain-dd-file").forEach(el => {
      const lab = (el.textContent || "").replace(/\s+/g, " ").trim(), tip = el.title || "";
      items.push({ el, mid: "chainDd", pan: "Цепочки ▾", lab, tip, nLab: norm(lab), nAll: norm(lab + " " + tip + " цепочки") });
    });
    return items;
  }
  function render(){
    const q = norm(inp.value);
    if (!q) { found = []; out.innerHTML = ""; return; }
    const words = q.split(" ");
    found = collect().filter(it => words.every(w => it.nAll.includes(w)))
      .map(it => ({ it, score: words.reduce((a, w) => a + (it.nLab.includes(w) ? 2 : 0), 0) + (it.nLab.startsWith(words[0]) ? 1 : 0) }))
      .sort((a, b) => b.score - a.score).map(x => x.it);
    const esc = s => s.replace(/[&<>"]/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[c]);
    out.innerHTML = found.length
      ? found.slice(0, 80).map((it, i) => '<div class="bf-row" data-i="' + i + '" title="' + esc(it.tip) + '"><button class="bf-go" data-go="' + i + '" title="Нажать">▶</button>' +
          '<span class="bf-lab">' + esc(it.lab) + '</span><span class="bf-pan">' + esc(it.pan) + '</span>' +
          (it.tip ? '<span class="bf-tip">' + esc(it.tip.slice(0, 140)) + '</span>' : "") + "</div>").join("") +
        (found.length > 80 ? '<div class="bf-info">… ещё ' + (found.length - 80) + ' — уточните запрос</div>' : "")
      : '<div class="bf-info">Ничего не нашлось</div>';
  }
  function reveal(it){
    const mid = it.mid;
    if (mid === "chainDd") { const dd = document.getElementById("chainDdList"); if (dd) dd.classList.add("open"); }
    else if (typeof panelInPopup === "function" && panelInPopup(mid)) focusPanelInPopup(mid);
    else if (!(typeof isTabVisible === "function" && isTabVisible(mid))) {
      const drop = document.getElementById(mid), btn = document.querySelector('.menu-btn[data-menu="' + mid + '"]');
      if (typeof closeMenus === "function") closeMenus();
      if (drop) { drop.style.left = Math.min(btn ? btn.offsetLeft : 0, Math.max(0, window.innerWidth - 330)) + "px"; drop.classList.add("open"); }
      if (btn) btn.classList.add("open");
    }
    const el = it.el;
    try { el.scrollIntoView({ block: "center", behavior: "smooth" }); } catch (e) { el.scrollIntoView(); }
    el.classList.add("bf-flash");
    setTimeout(() => el.classList.remove("bf-flash"), 1800);
  }
  function press(it){
    reveal(it);
    const el = it.el;
    if (el.tagName === "LABEL") { const cb = el.querySelector('input[type="checkbox"]'); if (cb) cb.click(); return; }
    if (el.disabled) { say("«" + it.lab + "» сейчас неактивна — нажать нельзя."); return; }
    el.click();
  }
  inp.addEventListener("input", render);
  inp.addEventListener("keydown", e => {
    if (e.key === "Enter" && found.length) { e.preventDefault(); (e.ctrlKey || e.metaKey ? press : reveal)(found[0]); }
    else if (e.key === "Escape") { inp.value = ""; render(); }
  });
  out.addEventListener("click", e => {
    const go = e.target.closest("[data-go]");
    if (go) { e.stopPropagation(); const it = found[+go.dataset.go]; if (it) press(it); return; }
    const row = e.target.closest(".bf-row"); if (!row) return;
    const it = found[+row.dataset.i]; if (it) reveal(it);
  });
  // Ctrl+K — открыть окно и встать в строку поиска
  document.addEventListener("keydown", e => {
    if (!(e.ctrlKey || e.metaKey) || e.altKey || !(e.key === "k" || e.key === "K" || e.key === "л" || e.key === "Л")) return;
    e.preventDefault();
    const mid = "menuFind";
    if (typeof panelInPopup === "function" && panelInPopup(mid)) focusPanelInPopup(mid);
    else if (!(typeof isTabVisible === "function" && isTabVisible(mid))) {
      const drop = document.getElementById(mid), btn = document.querySelector('.menu-btn[data-menu="' + mid + '"]');
      if (typeof closeMenus === "function") closeMenus();
      if (drop) { drop.style.left = Math.min(btn ? btn.offsetLeft : 0, Math.max(0, window.innerWidth - 330)) + "px"; drop.classList.add("open"); }
