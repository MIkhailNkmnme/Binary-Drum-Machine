...
/* ═══ ПОЛЗУНОК 🔢 СЧИТАЕТ ВСЕ СТРОКИ ЦЕПОЧКИ, А НЕ ШАБЛОН (v1.544) ═══
   Запрос пользователя: «раньше выше нулевой не было строк, и построений новых среди строк не
   было, — но они, даже вписанные, должны быть как родные и считаться».
   Было: предел ползунка = длина ШАБЛОНА (syncRowCountToTpl при загрузке), и дальше ползунок жил
   сам по себе. Цепочку достроили кнопками (🔺+1, 🔢+1, вставки) — строк стало больше предела,
   а ползунок стоял на старом числе: хвост за пределом им было не спрятать, и число врало.
   Стало: сверка в начале КАЖДОГО render(). Значение = сколько строк цепочки видно сейчас (от
   нулевой вниз; построения над нулевой не в счёт — их ползунок и раньше не резал), предел =
   видно + спрятано в стопке, но не меньше шаблона (выше него ползунок по-прежнему донабирает
   строки из шаблона). Откуда строка взялась — неважно: вписанная считается так же, как родная.
   rowCountApplied ставится тут же — от него applyRowCountDelta считает разницу. */
/* ═══ ЧИСЛО СТРОК ДО ВСТРОЙКИ — ЧТОБЫ «СБРОС» ВЕРНУЛ ЕГО (v1.561) ═══
   Запрос пользователя: «после вставки, а потом Сброс, число строк в ползунке надо восстановить,
   какое было до вставки». Ползунок сверяется с цепочкой (rowCountSync), поэтому после построений
   он честно показывает раздутое число — а Сброс берёт предел ИЗ ползунка и восстанавливал цепочку
   уже в новом размере. Теперь перед ПЕРВОЙ встройкой запоминается, сколько строк было; Сброс
   возвращает это число и отметку снимает. Отметка одна на серию: жми 🔺+1 хоть сто раз — Сброс
   вернёт то, что было до первого нажатия. */
var rowCountBase = null;
function rowCountMarkInsert(){
  if (rowCountBase !== null) return;
  rowCountBase = Math.max(1, (st.rows || []).length - (st.topBuilt || 0));
}
function rowCountSync(){
  const el = document.getElementById("rowCount");
  if (!el) return;
  const built = st.topBuilt || 0;
  const cur = Math.max(0, (st.rows || []).length - built);
  const full = Math.max(2, cur + rowCountStash.rows.length, (st.tplRows || []).length);
  if (+el.max !== full) el.max = full;
  if (+el.value !== cur) el.value = cur;
  rowCountApplied = cur;
  const v = document.getElementById("rowCountVal");
  if (v && v.textContent !== String(cur)) v.textContent = cur;
}
if (rowCountEl) {
  rowCountEl.oninput = () => {
...
const MENUS = {
  ...
  // «Перенос» (v1.457, запрос пользователя «отдели в отдельное меню»): блок ✂ переноса строк
...
/* Снимок раскладки панелей ОТДЕЛЬНО ОТ ЗАПИСИ (v1.452): тот же объект, что уходит в LAYOUT_KEY,
   но его теперь берёт себе ещё и «💾 Сохр. настройки» — чтобы «↺ Сброс» вернул панели ровно туда,
   где они стояли в момент сохранения (см. saveUiSettingsNow/resetUiSettingsNow в fold-5-ui.js).
   Ключ пишется как и прежде, на каждое движение панели; снимок живёт своей жизнью в кэше и
   перезаписывается только кнопкой. */
...
function updateToggleAllPinsBtn() {
  ...
  /* Признак «панелей на экране нет вовсе» (v1.165) — по нему в полосе выравниваний показывается
     ...
     пустой. Кнопка нужна в обоих случаях, поэтому признак считается по видимым вкладкам. */
  document.body.classList.toggle('no-panels', !anyOn);
...
function toggleAllPins() {
  ...
  /* ═══ БОКОВЫЕ ДОКИ УХОДЯТ И ВОЗВРАЩАЮТСЯ ВМЕСТЕ СО ВСЕМ ОСТАЛЬНЫМ (испр. v1.351) ═══
     Баг-репорт: «не всегда возвращает боковые панели клик колёсиком — только после нажатия кнопки
     начинает возвращать».
     Состояний оказалось ДВА, и щёлкали они порознь. Это (all-hidden + сами закрепления вкладок) и
     класс hide-side, который отдельно заводился в v1.163 под ту же среднюю кнопку. Клик колёсиком
     дёргал оба независимо: одно показывал, другое прятал — и на глаз «ничего не вернулось». Стоило
     нажать «▤ Показать панели», та выставляла hide-side явно, состояния совпадали, и следующий
     клик колёсиком снова выглядел рабочим. Отсюда и «только после нажатия кнопки».
     Держим hide-side ровно в том же положении, что и all-hidden: решение о показе/скрытии одно, и
     принимается оно здесь. Отдельный переключатель из обработчика средней кнопки убран (см.
     fold-4-tools.js) — там осталось только сообщение по факту.
     Кнопка «▤ Панели» (#bSide) по-прежнему правит ТОЛЬКО hide-side, и это осознанно: у неё своя
     задача — убрать доки, не трогая вкладок. Разойтись состояниям она может, но клик колёсиком
     теперь в любом случае сводит их к одному. */
...
/* ═══ ПРИМЕНЕНИЕ РАСКЛАДКИ ВЫНЕСЕНО ИЗ ЗАГРУЗКИ (v1.452) ═══
   Тело бывшей loadLayout(): раньше оно исполнялось один раз, прямо при разборе файла, и вернуть
   раскладку по ходу работы было нечем. Теперь это функция, а загрузка — её первый вызов; второй
   зовёт «↺ Сброс настроек», когда возвращает сохранённый снимок.
   Ничего в теле не изменено, кроме источника: L приходит параметром, а не читается из ключа.
   ВИДИМОСТЬ ОКОН при этом только записывается в overlayHidden; разложить её по DOM обязан
   вызывающий (при загрузке это делает отдельный проход ниже, при сбросе — setOverlayHidden на
   каждое окно, см. resetUiSettingsNow). */
...
function syncAlignBanned(){
  ...
  if (!grp) return;
  /* ЗАПРЕТ СЧИТАЕТСЯ ПО ЦЕПОЧКЕ (v1.213). До этого он считался «по приёмнику» полосы, но приёмник
     закреплён за цепочкой с v1.143, а в v1.213 убран совсем — значит поле у всех кнопок одно. */
  ...
  /* ЗДЕСЬ ЖИЛ ПОКАЗ ПРИЁМНИКА (v1.003 — v1.088): текст и подсветка кнопки #bAlignTargetInd, а
     после её удаления — подсветка подписей планок «П1»/«Ц»/«П2», по которой было видно, какое поле
     правит полоса. Приёмника больше нет (v1.213), показывать нечего. */
}
...
function overlayTopBase(){
  ...
  /* Замер содержимого полосы (высота заголовка и ряда вкладок) удалён в v1.391 как мёртвый: он
     нужен был только тому компромиссу, что стоял ниже, а отступ теперь считается по самой полосе.
     Читателей у него больше не было. */
  /* ═══ НАДГОРИЗОНТАЛЬНОЕ НАЧИНАЕТСЯ ПОД МЕНЮ 1, А НЕ ВПОЛОВИНУ ПОД НИМ (испр. v1.391) ═══
     Запрос пользователя: «когда там надгоризонтом строятся — учитывать отступ от меню 1».
     Здесь стоял компромисс: середина между высотой полосы и высотой её содержимого. Он и был
     «вполовину под меню» — стопка баров и всё, что строится над горизонтом, заезжало под верхнюю
     полосу тем глубже, чем та выше собственных кнопок. Пока полоса была в 42px, это давало
     видимость аккуратного зазора; с v1.391 её рост равен меню 2, и заезд стал заметен сразу.
     Отступаем на ВСЮ высоту полосы плюс общий зазор стопки. Полоса плавающая (position:fixed) и
     места в потоке не занимает, поэтому этот отступ — единственное, что удерживает содержимое
     из-под неё.
     Ветка «содержимого нет» отдавала 6 + barH, то есть по сути то же самое; сводим обе к одной
     формуле, чтобы не гадать, какая из них сработала. */
  return Math.round(barH + OVERLAY_GAP);
}
function layoutOverlayBoxes(){
  ...
  /* ═══ МЕНЮ 1 РОСТОМ С МЕНЮ 2 (v1.391) ═══
     Запрос пользователя: «меню 1 — высоту такую же, как и меню 2».
     Верхняя полоса держалась на своих 42px из :root, заведённых на глаз, и рядом с подтянувшимся
     меню 2 выглядела вдвое толще. Берём её высоту оттуда же, откуда и у планок полей (--strip-h,
     строкой выше): у трёх рядов управления рост становится общим и едет за кеглем сам.
     КНОПКАМ ВНУТРИ — та же мерка минус пара пикселей на собственные поля полосы, иначе они
     распирают её изнутри и высота перестаёт слушаться. Нижний упор 16px: ниже подписи вкладок уже
     не читаются, а полоса при мелком кегле может стать совсем тонкой.
     Обе величины пишем каждым проходом — как и всё остальное здесь, они должны ехать за кеглем и
     межстрочным, а не застывать на числе первого кадра. */
  if (alignH) {
    const bar1H = Math.max(16, alignH);
    document.documentElement.style.setProperty("--menubar-h", bar1H + "px");
    document.documentElement.style.setProperty("--menubar-btn-h", Math.max(14, bar1H - 4) + "px");
  }
  ...
  if (alignH) {
    ...
    /* ═══ ПРОСВЕТ ПОД ПОЛОСОЙ УБРАН (v1.322) ═══
       Запросы пользователя, два подряд и про одно: «убери отступ от нижней границы меню
       выравниваний от строк цепочек, 2px сделай» и «разрыв горизонта больше не нужен».
       Было (v1.261/1.266): высота полосы + 4px + ДВЕ СТРОКИ просвета. Две строки были
       заведены под ось цепочки — чтобы та не уходила полосе за спину.
       Стало: ровно 2px, то есть строки начинаются сразу под полосой. Оси при этом остаётся
       её собственный отрезок вниз, на нулевую строку (v1.302, там битов не бывает), а вверх она
       теперь не тянется: тянуться больше некуда, и просили именно этого.
       Обе переменные правятся вместе: --align-band-h — разрыв в строках (margin-top у строки
       горизонта, см. render()), --axis-gap-h — длина оси в этом разрыве (см. updateAxisSplitPosition
       в fold-5-ui.js, там же нижний упор 14px). Разойдутся — ось снова уйдёт за полосу. */
    /* ДВА ПИКСЕЛЯ СЧИТАЮТСЯ ОТ ПРЕЖНЕГО УРОВНЯ (испр. v1.323, баг-репорт «отступ от
       нижней границы меню выравнивания нужен 2px»). В v1.322 я поставил alignH + 2, то есть
       выбросил вместе с двумя строками просвета и базовые 4px, а это не зазор был, а выход полосы
       на свою нижнюю кромку: строки упёрлись в полосу вплотную. Возвращаем 4px как уровень
       «впритык» и добавляем ровно те 2px, которые просили. Двух строк просвета как не было,
       так и нет — разрыв горизонта убран в v1.322 и не возвращается. */
    /* ═══ В РАЗРЫВЕ СНОВА ЕСТЬ МЕСТО — РОВНО ПОД РЯД ОСИ (испр. v1.326) ═══
       Запрос пользователя: «сделай ось цепочек такой высоты, чтобы кнопки не наезжали на паттерны».
       В v1.322 разрыв был сжат до голой полосы, и коробкам у оси (пикеры, счётчики, замок)
       ставиться стало некуда: сначала они лезли на полосу (v1.324), потом — вытолкнутые вниз
       упором — на паттерны. Место им нужно СВОЁ, и вот оно.
       Разрыв = полоса (alignH) + 4px «впритык» + 2px зазора, как просили в v1.323, + ОДИН ряд
       коробок. Высота ряда считается тем же правилом, что и --axis-row-h в CSS (v1.325):
       большее из высоты полосы и 20px — собственного роста счётчиков лесенки. Два места,
       где это число живёт, обязаны совпадать: разойдутся — или снова наложатся, или останется дыра.
       --axis-gap-h — длина самой оси; прибавляем 6px, потому что updateAxisSplitPosition() вычитает
       ровно столько (v1.264, чтобы линия не упиралась макушкой в полосу) — то есть ось выйдет
       ровно в ряд коробок, а коробки — по её середине. */
    /* ═══ ДВЕ ПУСТЫЕ СТРОКИ ПОД ОСЬ И ЕЁ КНОПКИ (v1.327) ═══
       Запрос пользователя: «ось цепочек пусть занимает 0 и над ней ещё строку, и на уровне
       этих двух пустых, всегда свободных от битов, расположи кнопки слева и справа от оси».
       В v1.326 я выделил коробкам ряд в пикселях (max(alignH, 20)) — он не был кратен строке,
       и низ оси со своей прибавкой в одну строку (v1.302) доходил до первой строки с битами.
       Теперь ряд ровно В СТРОКУ (pitchPx): вместе с нулевой строкой, которая идёт сразу
       за разрывом и битов не несёт, получаются те самые ДВЕ пустые строки. Ось стоит в обеих
       (верхняя половина — этот разрыв, нижняя — прибавка v1.302), коробки — по середине
       этих двух строк, то есть ни на полосу, ни на биты не попадают ни при каком кегле.
       Строка узкая (мелкий кегль) — берём не меньше 20px на две, иначе коробки со своим
       собственным ростом выйдут за две строки и всё равно на что-нибудь налягут. */
    /* ═══ РЯД ПОД КОРОБКИ БОЛЬШЕ НЕ НУЖЕН (v1.353) ═══
       Запрос пользователя: «убери отступ от 0 строки и меню» — вместе с «кнопки у оси перемести
       сюда», в полосу.
       Этот ряд (axisRowH) и БЫЛ тем отступом. Его завели в v1.326/v1.327 под коробки у оси —
       пикеры цвета, замок, счётчики лесенок: им нужно было место, где они не лягут ни на полосу,
       ни на биты. Пикеры и замок переехали в саму полосу (см. переезд в fold-5-ui.js), и держать
       под них пустую строку стало незачем.
       Остаётся ровно то, что просили ещё в v1.323: полоса, 4px «впритык» и 2px зазора. Нулевая
       строка начинается сразу под полосой.
       --axis-gap-h ставим по прежней формуле от axisRowH: это ЗАПАСНАЯ длина оси, она работает
       только когда черты горизонта на экране нет (см. axisLineH в updateAxisSplitPosition, v1.348);
       при живой черте длина считается от неё и этой переменной не спрашивает.
       СЧЁТЧИКИ ЛЕСЕНОК теперь встают по середине укоротившегося отрезка оси — то есть на нулевую
       строку. Битов там не бывает по построению, так что накрыть им нечего. */
    /* ═══ НУЛЕВАЯ СТРОКА ЕСТЬ И У НАДГОРИЗОНТАЛЬНЫХ (v1.384) ═══
       Запрос пользователя: «для надгоризонтальных сделай 0 строку и продли на неё ось цепочек; всё,
       что между 0 и 0 — там и меню 2 — другим фоном залей».
       У цепочки нулевая строка есть всегда: она идёт сразу под разрывом и битов не несёт (см.
       ensureZeroRow), поэтому ось спокойно заходит на неё (v1.302). У поля наложений такой строки
       не было — блоки кончались прямо у разрыва, и сверху ось упиралась в них.
       Открываем ряд и над разрывом: прибавляем к нему один шаг строки. Снизу разрыв по-прежнему
       кончается там же, где начинается нулевая строка цепочки, значит прибавка уходит ВВЕРХ — ровно
       туда, где нужна пустая строка наложений.
       ОСЬ ПРОДЛЕВАЕТСЯ САМА. Её длина считается от черты горизонта до низа разрыва (v1.348), а
       черта стоит на его верхней кромке — разрыв вырос, и ось вместе с ним. Отдельной правки не
       нужно, и второго места, где эту длину надо помнить, не заводится. */
    /* ═══ РАЗРЫВА НЕТ ВОВСЕ: МЕНЮ 2 И ЕСТЬ НУЛЕВАЯ СТРОКА (v1.403) ═══
     Запрос пользователя: «убери над 0 строкой пустую строку, оставь только меню 2, и саму 0 строку
     тоже — теперь меню 2 это 0 строка».
     Разрыв был складом пустого места: строка наложений сверху (v1.384), сама полоса, пара пикселей
     зазора, — и всё это НАД нулевой строкой цепочки, которая тоже пустая по построению. Итого три
     пустых ряда подряд там, где просили один.
     Ставим ноль. Место под полосу больше не открывается: с v1.402 она ростом ровно в строку, лежит
     абсолютом и садится ПОВЕРХ нулевой строки — а та битов не несёт никогда, закрывать ей нечего.
     Ни удалять нулевую строку из данных, ни прятать её в разметке не пришлось: нумерация,
     зеркала и вся геометрия, которая меряется по первой .ln, остались нетронутыми. Изменилось
     только то, что на её месте теперь видно полосу.
  ...
  /* ═══ ВЕРХНИЙ ПРЕДЕЛ ГОРИЗОНТА — ПОД МЕНЮ 1 ПЛЮС 2px (v1.392) ═══
     Запрос пользователя: «для горизонта верхний предел — это не как сейчас, а ниже меню 1 на отступ
     ещё 2 пикселя».
     Черта горизонта стоит НАД строками, на высоту разрыва (--align-band-h: меню 2 плюс пустая
     строка наложений, v1.384). Отступ холста сверху при этом считался только по стопке баров, а
     когда её нет — просто 8px. Значит при пустой стопке строки начинались почти у самой кромки, а
     черта, которая выше них на целый разрыв, оказывалась ЗА НЕЙ — под верхней полосой, куда её ни
     тащи. Верхнего предела как такового не было вовсе: его роль играл случайный остаток.
     Задаём предел прямо: отступ холста не меньше, чем «высота меню 1 + 2px + разрыв». Тогда самая
     верхняя точка, куда может подняться черта, — ровно на два пикселя ниже меню 1, как и просили.
     Меню внизу экрана (menubar-bottom) — сверху полосы нет, и предел вырождается в те же 2px плюс
     разрыв.
     Считаем ЧЕРЕЗ МАКСИМУМ, а не заменой: когда стопка баров высокая, она и так отжимает холст
     ниже предела, и портить это правило незачем. */
  const anyOverlayBars = visible.length > 0;
  const cssNum = (name, dflt) => {
    const v = parseFloat(getComputedStyle(document.documentElement).getPropertyValue(name));
    return isFinite(v) ? v : dflt;
  };
  /* ═══ РАЗРЫВ СВЕРХУ НЕ РЕЗЕРВИРУЕТСЯ (испр. v1.392 → v1.393) ═══
     Баг-репорт: «примерно 5 строк там расстояние».
     Я прибавил к отступу холста высоту разрыва, решив, что черта стоит НАД строками на эту
     величину. Она стоит НА ИХ ВЕРХНЕЙ КРОМКЕ: см. updateTopHorizon — top черты равен верху #rows
     (плюс h высот строки при поднятом горизонте). А сам разрыв — это margin-top первой строки, то
     есть он ВНУТРИ #rows и ниже черты. Прибавка была чистым перебором, и на экране это дало
     лишний ряд меню 2 плюс пустую строку — те самые «примерно 5 строк» вместе с линейкой.
     Считаем честно: до верха #rows от кромки холста лежит ещё шапка колонок с линейкой. Меряем эту
     величину живьём и вычитаем — тогда сам #rows (а значит и черта) встаёт ровно на «меню 1 плюс
     два пикселя», а не ниже на высоту линейки.
     Замер до записи стиля: отступ холста, который мы сейчас поставим, на расстояние «от .chain до
     #rows» не влияет — оно внутреннее. Петли нет. */
  const bar1ForTop = document.body.classList.contains('menubar-bottom') ? 0 : cssNum('--menubar-h', 42);
  const headOffsetForTop = (() => {
...
function positionAlignGrpTop(){
  ...
  if (el) {
    ...
    /* ═══ МЕНЮ 2 НЕ ЗАЕЗЖАЕТ ПОД МЕНЮ 1 (испр. v1.450) ═══
       ...
       Потолок (ceil выше) считался от --menubar-h — переменной, которую ставит другой проход по
       измеренной высоте полосы. Величина ОТСТАЮЩАЯ: полоса меню 1 растёт от кегля и от переноса
       вкладок на второй ряд, и в кадре, где она уже выше, переменная ещё вчерашняя — меню 2 в этот
       момент и залезает под неё. Дальше кадры выравниваются, но глазу хватает и одного.
       Меряем полосу НАПРЯМУЮ и не пускаем меню 2 выше её низа плюс два пикселя — тот самый зазор,
       который и просили. Замер живой, отставать ему не от чего.
       Меню внизу экрана (menubar-bottom) — упора нет: там над меню 2 ничего не висит, а прижимать
       его вниз этот расчёт не должен. */
    if (!document.body.classList.contains("menubar-bottom") && host) {
      const barLive = document.getElementById("menuBar");
...
document.getElementById("rows").onclick = e => {
  ...
  /* ═══ КОЛОНКА НОМЕРОВ ЦЕПОЧКИ СТРОКУ НЕ ВЫДЕЛЯЕТ (v1.422) ═══
     Запрос пользователя: «клик по номеру Ц выделяет строку — отключи одинарный».
     С v1.419 у этой колонки появилось своё действие: двойной клик меняет систему счисления
     (10 ⇄ 2). Одиночный при этом доставался строке и переставлял выделение — то есть пока
     целишься во второй щелчок, первый успевает увести выбор на другую строку.
     Клик по номерам теперь не делает ничего: колонка стала местом ОДНОГО жеста, двойного.
     Выделять строку по-прежнему можно кликом по её битам или по пустому месту рядом — колонка
     номеров узкая, и потерей это не назовёшь.
     Проверку ставим раньше всех прочих, как и у вставки выше: дальше по обработчику идут ветки
     режимов (ячейки, выбор столбца, замок), и каждой пришлось бы объяснять то же самое отдельно.
     dblclick это не мешает: он приходит своим событием и своим слушателем (fold-5-ui.js). */
  if (e.target.closest && e.target.closest(".num-l2")) { e.stopPropagation(); return; }
  ...
  /* Под замком (🔒) выделение выключено — КРОМЕ ячеек паттернов (v1.130): их выбор ничего не
     правит, он лишь указывает, что искать в цепочке, и запирать его вместе с правкой незачем.
     См. patsSelectAllowed() в fold-1-core.js. */
  if (!selectionAllowed() && !(e.target.closest && e.target.closest(".pat, .pat2"))) return;
  ...
  if (e.target.closest(".pat, .pat2")) {
    // Под замком (🔒) — ячейки паттернов не выделяются вовсе (отдельный режим «🔏» убран в v1.112),
    // хотя строки цепочки рядом продолжают работать. См. patsEditAllowed() в fold-1-core.js.
    if (!patsSelectAllowed()) return;   // v1.130: выбор паттернов замок не запирает
    ...
    if (st.pats[idx]) {
      ...
      /* Парная половина правила «выделение одно на двоих» (v1.378, разбор — у выделения строк
         ниже по файлу): взялись за паттерны — отпускаем строки цепочки. */
      if (st.selectedRows && st.selectedRows.size) {
        st.selectedRows.clear();
        if (typeof updateVariantCounter === "function") updateVariantCounter();
      }
  ...
  /* НАД ЛИНИЕЙ СТРОКИ НЕ ВЫДЕЛЯЮТСЯ (v1.195, запрос: «над горизонтом строки вообще никогда не надо
     выделять, только паттерны можно»).
     Верхнее поле — рабочая площадка наложений: собственных бит у его строк нет, в расчёт они идут
     только через блоки, и выделять там нечего. А вот ЯЧЕЙКИ ПАТТЕРНОВ выделять надо — ими задаётся
     цель поиска, — поэтому запрет стоит здесь, в ветке выделения СТРОК: до колонок паттернов
     обработчик к этому месту уже не доходит, у них своя ветка выше. */
  if (typeof horizonOn === "function" && horizonOn() && idx < horizonRow()) {
    /* ...И СНИМАЕТ ТО, ЧТО БЫЛО ВЫДЕЛЕНО ВНИЗУ (v1.213, запрос пользователя: «убирай выделение
       строк в цепочках при любом клике выше горизонта»).
       Раньше клик наверху просто выходил, и выделение внизу оставалось нетронутым — а оно держит
       весь контекст работы (фон-поиск смотрит на строку под выделенной, «Авто» крутит выделенные,
       построения строятся по ним). Человек уходит работать с блоками в верхнее поле, а внизу
       продолжает жить прежний выбор, и первое же нажатие ◄/► или пробел бьёт совсем не туда, куда
       он смотрит. Клик наверху теперь значит «я здесь» — цепочка отпускается. */
    if (st.selectedRows && st.selectedRows.size) {
      st.selectedRows.clear();
      if (typeof updateVariantCounter === "function") updateVariantCounter();
      render(); saveCache();
  ...
  /* ═══ НУЛЕВАЯ СТРОКА НЕ ВЫДЕЛЯЕТСЯ (v1.406) ═══
     Запрос пользователя: «не давай 0 строку выделить».
     Битов она не несёт никогда (см. ensureZeroRow), а с v1.403 её место занимает меню 2 — то есть
     клик «по нулевой строке» это почти всегда промах мимо кнопки полосы. Выделять при этом нечего:
     все операции, которые смотрят на выделение (Круг, построения, «Авто», фон-поиск), на пустой
     строке молчат, а вид у неё при этом становится «выбрана» — и следующий Delete или ◄/► бьёт не
     туда, куда человек смотрит.
     Молча выходим, не трогая уже набранное выделение: клик по полосе не должен ни выделять нулевую,
     ни сбрасывать то, что выбрано ниже.
     Ячейку ПАТТЕРНА у нулевой строки это не касается — там выделение осознанно разрешено (v0.9xx,
     «чтобы в паттернах тоже можно было выбрать 0 строку»), и удаляют её именно так. */
  /* ═══ ЗАПРЕТ СНЯТ (v1.569) ═══
     Запрос пользователя: «какая-то полоса остаётся и не даёт выделить 0 строку» (второй раз подряд).
     Причина запрета v1.406 ушла: меню 2 больше не глотает клики по строке (v1.566), значит клик
     по нулевой — это клик по нулевой, а не промах мимо кнопки. Нулевая выделяется, как все.
     Проверка оставлена выключенной строкой ниже — вернуть запрет значит раскомментировать её. */
  // if (typeof rowLabel === "function" ? rowLabel(idx) === 0 : idx <= 0) return;
  markLastSel("rows");     // v1.229: см. lastSelKind в fold-1-core.js
  /* ═══ ВЫДЕЛЕНИЕ ОДНО НА ДВОИХ: СТРОКИ ГАСЯТ ПАТТЕРНЫ (v1.378) ═══
     Запрос пользователя: «снимай выделение строк с паттернов, когда выделил в цепочках, и
     наоборот».
     Два набора (st.selectedRows и st.selectedPats) заводились независимыми намеренно — они и
     значат разное: строки задают, ЧТО крутить и достраивать, паттерны сужают, ЧТО искать. Но на
     экране оба горят одинаковой подсветкой, и, оставаясь вдвоём, они разводят руку с глазом:
     выделил строку внизу, а «🌈 Все паттерны» по-прежнему сужен вчерашним выбором наверху, и
     непонятно, почему находок меньше, чем видно.
     Гасим второй набор при каждом новом выделении в первом — и наоборот (парная правка у ячеек
     паттернов выше). Гасим ДО того, как трогаем свой: порядок не важен для результата, но так
     видно, что это одно действие, а не два.
     Ctrl-клик исключением не делаем: он добавляет к СВОЕМУ набору, а чужой всё равно относится к
     другой работе, и держать его включённым «на всякий случай» — ровно та путаница, о которой речь.
     lastSelKind (v1.229) трогать не нужно: он и так уже переставлен на «rows» строкой выше — то
     есть Delete теперь смотрит сюда, что с этой правкой окончательно совпало. */
  if (st.selectedPats && st.selectedPats.size) st.selectedPats.clear();
...
document.getElementById("rows").addEventListener("mousedown", e => {
  ...
  /* Колонка номеров цепочки не начинает и протяжку выделения (v1.422, парно к клику выше): нажатие
     на ней — это первая половина двойного клика, а не начало выбора диапазона. Без этой проверки
     дрожь руки между двумя щелчками успевала бы протянуть выделение по строкам. */
  if (e.target.closest && e.target.closest(".num-l2")) return;
  // Под замком протяжка по СТРОКАМ не выделяет, а по ячейкам ПАТТЕРНОВ — выделяет (v1.130,
  // запрос «при Замке дай возможность выделять паттерны»): она их не правит, только указывает,
  // что искать. См. patsSelectAllowed() в fold-1-core.js.
  if (!selectionAllowed() && !(e.target.closest && e.target.closest(".pat, .pat2"))) return;
  ...
  if (e.target.closest(".pat, .pat2")) {
    // Под замком (🔒) якорь не ставим, значит и протяжки по колонке не будет (v1.057, 🔏 убран в v1.112).
    // return без якоря, а не проверка внутри mousemove: так жест просто не начинается.
    if (!patsSelectAllowed()) return;   // v1.130: протяжка по ячейкам паттернов — тоже выделение, замок её не запирает
    patDragAnchor = idx;
    patDragMoved = false;
    dragOverRowsOn();
    return;
...
window.addEventListener("mousemove", e => {
  ...
  /* Протяжка тоже не берёт строки над линией (v1.195): диапазон может начаться ниже и уехать
     вверх, и без этой проверки поле выделялось бы «заодно». */
  /* Нижняя граница набора — не меньше первой строки (v1.406): нулевую выделять нельзя и
     протяжкой тоже, разбор — у клика по строке выше. Единица, а не rowFloor: тот про горизонт,
     и когда поле наложений закрыто, он равен нулю. */
  /* Пол набора: сама нулевая строка не выделяется (v1.406), поэтому начинаем со следующей за ней.
     Её МЕСТО в массиве считаем через горизонт: строка с номером 0 — первая под ним (v1.408, там же
     разбор, почему индекс и номер это разные вещи). */
  const zeroIdxSel = (typeof horizonOn === "function" && horizonOn() && typeof horizonRow === "function")
    ? horizonRow() : 0;
  const rowFloor = zeroIdxSel;   // v1.569: нулевая снова выделяется (запрет v1.406 снят) — было zeroIdxSel + 1
  for (let i = Math.max(lo, rowFloor); i <= hi; i++) st.selectedRows.add(i);
  render();
});
window.addEventListener("mouseup", () => {
...
document.getElementById("rows").addEventListener("dblclick", e => {
  ...
  /* НАД ГОРИЗОНТОМ ДВОЙНОЙ КЛИК МОЛЧИТ (v1.211, запрос пользователя: «запрети двойной клик над
     горизонтом — кроме паттернов, там он снимет выделения сразу все»).
     Выше линии живёт поле наложений: там блоки таскают, ставят и подгоняют мышью, и второй клик
     подряд запросто случается сам собой — а открывал он РУЧНУЮ ПРАВКУ строки, то есть менял
     содержимое цепочки там, где человек всего лишь двигал блок.
     Проверка стоит ПОСЛЕ ветки колонок паттернов намеренно: в П1/П2 двойной клик по-прежнему
     снимает выделение со всех паттернов разом — над горизонтом в том числе. */
  if (typeof horizonOn === "function" && horizonOn() && idx < horizonRow()) return;
...
alignBtns.forEach(btn => {
  btn.onclick = () => {
    ...
    /* ПОЛОСА ВСЕГДА ПРАВИТ ЦЕПОЧКУ (v1.143, окончательно — v1.213). Кнопка «◧ П1»/«П2 ◨»-приёмник
       ...
       приёмник. Выравнивание колонок — кнопки «⇤» в их планках (cyclePatAlign в fold-5-ui.js). */
    ...
    if (st.selectedRows && st.selectedRows.size > 1 && !isAxisAlign(valNow)) {
    ...
    /* ЗДЕСЬ БЫЛА ВЕТКА «ПОЛОСА ПРАВИТ КОЛОНКУ П1/П2» (v0.976) — она читала приёмник и клала
       значение в patAlign/pat2Align. С v1.143 приёмник закреплён за цепочкой, и ветка стала
       недостижимой; в v1.213 убрана вместе с ним. Колонки ровняют кнопки «⇤» их планок. */
    ...
    /* ЗДЕСЬ БЫЛА ПРОВЕРКА ЗАМКА ДЛЯ ПОЛОСЫ, НАПРАВЛЕННОЙ НА П1/П2 (v1.213: убрана вместе с
       приёмником). Полоса до колонок паттернов больше не дотягивается вовсе, а свою проверку
       замка их собственная кнопка «⇤» держит сама (cyclePatAlign в fold-5-ui.js). */
...
/* ПОДПИСЬ КНОПКИ — САМО СОСТОЯНИЕ (v1.159, запрос пользователя: «меняй текст — Ищет, не ищет»).
   Раньше на кнопке всегда стояло «⛔ Паттерны выше выделенной — не искать», то есть было написано,
   что она СДЕЛАЕТ, а не что происходит сейчас. Понять по ней, ищутся паттерны выше отсечки или нет,
   можно было только по подсветке — а это не читается, когда рядом полтора десятка таких же кнопок.
   Теперь текст прямо говорит текущее положение дел: «ищет» или «не ищет». Подсветка остаётся, она
   дублирует то же самое цветом. */
function applyNoPatsAboveBtn(){
  if (!bNoPatsAboveEl) return;
...
/* ДОКУДА ИСКАТЬ ПАТТЕРНЫ ПРИ ВКЛЮЧЁННОЙ «⛔ Паттерны выше выделенной — не искать».
   Возвращает номер строки, НАЧИНАЯ с которой паттерны ещё ищутся; -1 — отсечки нет.
   С ГОРИЗОНТОМ ОТСЧЁТ ИДЁТ ОТ ЛИНИИ (v1.155, запрос: «если включено, то не искать паттерны,
   находящиеся в строках, участвующих в склейке над горизонтальной полосой»).
   Смысл галки — «не находить то, что уже ушло в склейку»: строки выше отсечки собраны в сквозную,
   и находить в ней их же собственные паттерны бессмысленно. Раньше границей была выделенная
   строка, но с поднятым горизонтом в склейку идёт ровно то, что ВЫШЕ ЛИНИИ, независимо от
   выделения, — значит и отсекать надо по ней. Заодно галка заработала под замком, где выделения
   нет вовсе и отсечка молча не срабатывала.
   Выделение остаётся главнее: горизонт подставляется, только когда выбирать не из чего. */
function patSearchFloorIdx(){
  if (!st.noPatsAbove) return -1;
  if (st.selectedRows && st.selectedRows.size) return Math.max(...st.selectedRows) + 1;
  if (typeof horizonOn === "function" && horizonOn()) return horizonRow();
  return -1;
}
function findAllPatternsInResult(text){
  ...
  /* ПОД ЛИНИЕЙ НЕ ИЩЕМ ВОВСЕ (v1.200, тот же запрос). Строки под горизонтом в расчёт не идут —
     getRowBits отдаёт их пустыми, — а вот их паттерны список перебирал по-прежнему, находил
     в верхнем поле и подсвечивал в колонке. Теперь перебор кончается на линии: над ней ищем
     всё, что там лежит, под ней — ничего. */
  const patCeil = (typeof horizonOn === "function" && horizonOn()) ? horizonRow() : -1;

  for (let i = 0; i < st.pats.length; i++) {
    if (patCeil >= 0 && i >= patCeil) continue;
...
function computeBgSearchTarget(){
  ...
  /* ГОРИЗОНТ САМ ЗАДАЁТ ЦЕЛЬ, ЕСЛИ ВЫДЕЛЕНИЯ НЕТ (v1.149, баг-репорт: «почему не строит сквозные
     и прочее из фонового поиска» — в панели при этом «нет цели»).
     Ориентир поиска всегда брался от ВЫДЕЛЕННОЙ строки, а под замком 🔒 выделения нет и быть не
     может: оно там выключено целиком. Отсюда и «нет цели» — считать было не от чего, и молча
     пустела вся панель результата.
     С поднятым горизонтом ориентир очевиден и без выделения: выше линии лежит всё, что участвует,
     значит целью становится первая строка ПОД ней. Так замок и горизонт наконец работают вместе,
     а не мешают друг другу.
     Выделение, если оно есть, по-прежнему главнее: горизонт подставляется только когда выбирать
     не из чего. */
  const hOn = (typeof horizonOn === "function") && horizonOn();
  /* ПОДНЯТЫЙ ГОРИЗОНТ ПЕРЕБИВАЕТ ВЫДЕЛЕНИЕ СТРОК (v1.200, запрос: «когда над горизонтом есть
     биты и сквозная — не искать и не выделять биты и паттерны под линией, работаем только над
     горизонтом»). Раньше горизонт подставлял цель, только когда выделения не было вовсе, и
     выделенная ВНИЗУ строка уводила поиск под линию — туда, где данные в расчёт уже не идут.
     Теперь при поднятой линии выделение строк на цель не влияет: целью становится нижняя
     строка поля (selIdx ниже), а сузить её можно выделением паттернов — но только тех, что
     лежат над линией (см. patCeil). */
  const hasSel = !!(st.selectedRows && st.selectedRows.size) && !hOn;
  if (!hasSel && !hOn) return null;
  const selIdx = hasSel ? Math.max(...st.selectedRows) : (horizonRow() - 1);
  ...
  /* С ПОДНЯТЫМ ПОЛЕМ ИЩЕМ ТОЛЬКО В НЁМ (v1.191, запрос: «когда над горизонтом есть биты, то
     паттерны искать только в них, ниже не ищем»).
     Обычная цель поиска — паттерн строки СРАЗУ НИЖЕ выделенной: ищем то, что «идёт следующим».
     Но когда работа идёт в верхнем поле, строка ниже линии принадлежит цепочке, и цель уезжала
     туда — то есть искали не то и не там. Теперь без выделения целью становится ПОСЛЕДНЯЯ СТРОКА
     ПОЛЯ: её паттерн — копия из цепочки (см. syncFieldPatterns), а искать его будут по склейке
     того, что лежит выше, то есть по самому полю. Строки под линией в склейку не попадают вовсе —
     getRowBits отдаёт их пустыми, — поэтому «ниже не ищем» выполняется само собой.
     Есть выделение — оно главнее, поведение прежнее: цель под выделенной строкой. */
  const fieldMode = !hasSel && (typeof horizonOn === "function") && horizonOn();
  const targetIdx = fieldMode ? selIdx : (selIdx + 1);
  ...
  const patFloor = patSearchFloorIdx();
  /* Потолок поля (v1.200): паттерн строки под линией целью стать не может — его строка в
     расчёте не участвует. Выделение под линией просто не считается, и цель остаётся за
     нижней строкой поля. */
  const patCeil = hOn ? horizonRow() : -1;
...
function autoRun(){
  ...
  if (st.lastDirMode) {
    ...
    if (isShift) {
      ...
      mirrorsBeforeShift();
      totalTurns = shiftModeTotal(rotIdxs);
      if (analysisRun && !totalTurns) { analysisRun.noEnd = [...rotIdxs].some(i => st.rows[i]) ? "big" : "empty"; return finishAuto(); }
    } else {
      const range = colSelectRowRange();
      const lo = Math.max(0, range.lo);
...
function analysisFinish(m){
  ...
  if (a.noEnd === "empty") return `🔬 Анализ (${tool}): в выделении нет ни одной непустой строки — крутить нечего.`;
...
/* ═══ СБРОС ЗАОДНО СТАВИТ ОСЬ НА МЕСТО (v1.456) ═══
   Запрос пользователя: «пусть также ставит в середину Ц ось и по экрану».
   После сброса к шаблону строки другие, а ось оставалась там, где её оставила прежняя цепочка:
   на глаз она оказывалась то у края поля, то вовсе за кромкой экрана, и первым делом приходилось
   вести её руками.
   Зовём тот же жест, что висит на двойном клике по самой оси (axisCenterInFieldAndScreen в
   fold-5-ui.js): ось встаёт в середину поля цепочки и выводится на середину окна шириной П1.
   Второй логики центрирования не заводим — один вход, одно поведение.
   Функция живёт в файле, который подключается ПОЗЖЕ этого, поэтому зовём её через typeof: на
   момент разбора этой строки её ещё нет, а на момент клика она уже есть.
   Своё сообщение она печатает сама, поэтому наше «Сброшено к шаблону» уходит ПЕРЕД ней. */
document.getElementById("bReset").onclick = () => {
  resetAll();
  say("Сброшено к шаблону.");
...
/* НАПРАВЛЕНИЕ (v1.553, запрос пользователя «и ещё кнопки нужны, чтоб достраивал не вниз, а вверх»).
   up = true — новая строка встаёт НАД источником, и всё, начиная с него, съезжает вниз; в режиме ⊕
   результат ложится в строку НАД источником. Остальное (выбор источника, перенос привязок,
   выделение на получившуюся строку) — общее, поэтому направление вошло сюда одним параметром. */
function insertNextBelow(tag, logName, make, up){
    let last;
    ...
    const out = made.out;
    const srcLabel = rowLabel(last);           // номер источника ДО вставки: после неё он съедет
    const idx = up ? last : last + 1;          // куда встанет новая строка
    const tgtIdx = up ? last - 1 : last + 1;   // куда лечь XOR-ом (строка над/под источником)
    /* ═══ ⊕ В СТРОКУ НИЖЕ (v1.549) ═══
       Запрос пользователя: «построение делать прямо на строках, кладя в биты (XOR) или между
       имеющихся». Второе — прежняя вставка, а здесь результат НЕ занимает своей строки: он
       XOR-ится в ту, что уже стоит под источником, бит к биту слева направо. Строки не двигаются,
       привязки по номерам не съезжают, и поэтому весь блок сдвига ниже тут не нужен.
       Строка ниже с точками/пустотами не годится: у них нет значения, которое можно XOR-ить
       (то же правило, что у остальных построений). Нет строки ниже или она пуста — обычная вставка.
       Новыми помечаются ровно те биты, что изменились, а не вся строка: остальные как стояли. */
    ...
    if (bmEl && bmEl.value === "xor") {
      ...
      say(`${tag} ⊕: ${up ? "над" : "под"} строкой ${srcLabel} пусто — класть XOR-ом не во что, вставляю новой строкой.`);
    }
    snapshot();
    rowCountMarkInsert();   // v1.561: запомнить размер до встройки — для Сброса
    ...
    say(`${tag}: ${up ? "над" : "под"} строкой ${srcLabel} (${show(src)}) вставлена ${show(out)}` +
...
if (bDescentEl) bDescentEl.onclick = () => {
  ...
  descentLast = { levels, label: rowLabel(r), edge, head: `строка ${rowLabel(r)} · ${n} бит · единиц ${sOnes}, в краю ${eOnes}` };
  descentPaint(true);
  descentShow();
  render();
  say(`▽ Спуск строки ${rowLabel(r)}: ${n} этаж., край — ${n} бит (единиц ${eOnes} против ${sOnes} в строке): ${verdict}. Треугольник — в окне «▽» (и в «🧾 Черновике шага»).`);
};

/* ═══ ОКНО «▽ СПУСК» (v1.546) ═══
   Последний спуск лежит в descentLast и рисуется в #descentView. Выравнивание СВОЁ, из списка
   #descentAlign, от цепочки НЕ зависит (уточнение пользователя: «стой, выравнивание надо разное,
   отдельное»). По умолчанию по центру: этажи короче на бит, и центр ставит каждый бит XOR ровно
   под его парой. descentPaint(force) без force перерисовывает, только если выравнивание сменилось. */
var descentLast = null;
var descentPaintedAl = "";
function descentAlignNow(){
  const sel = document.getElementById("descentAlign");
  return sel ? sel.value : "center";
}
function descentPaint(force){
  const view = document.getElementById("descentView");
  if (!view || !descentLast) return;
  const al = descentAlignNow();
  if (!force && al === descentPaintedAl) return;
  descentPaintedAl = al;
  view.className = "descent-view al-" + al;
  const L = descentLast.levels;
  let h = "";
  for (let k = 0; k < L.length && k < DESCENT_SHOW; k++) {
    const l = L[k];
    h += '<span class="dl">' + (k === 0 ? "строка" : "−" + k) + '</span><span class="dr">' +
         '<span class="descent-edge b' + l[0] + '" title="Край: первый бит этажа ' + k + '">' + l[0] + '</span>' + bitsHtml(l.slice(1)) + '</span>';
  }
  if (L.length > DESCENT_SHOW) h += '<span class="dl">…</span><span class="dr" style="font-size:11px;color:var(--dim)">ещё ' + (L.length - DESCENT_SHOW) + ' этаж. — край учтён целиком</span>';
  h += '<span class="dl">край</span><span class="dr edge-row">' + bitsHtml(descentLast.edge) + '</span>';
  view.innerHTML = h;
  const head = document.getElementById("descentHead");
  if (head) head.textContent = descentLast.head;
}
/* Показать окно, если его сейчас не видно: закреплённое или плавающее и так на виду, а выпадающее
   открываем тем же путём, что и клик по вкладке. */
function descentShow(){
  const mid = "menuDescent", m = MENUS[mid];
  if (!m) return;
  if (typeof panelInPopup === "function" && panelInPopup(mid)) { if (typeof focusPanelInPopup === "function") focusPanelInPopup(mid); return; }
  if (m.pin || (m.wrapper && m.wrapper.classList.contains("floating-panel"))) return;
  const drop = document.getElementById(mid);
  const btn = document.querySelector('.menu-btn[data-menu="' + mid + '"]');
  if (drop && btn && !drop.classList.contains("open")) btn.click();
}
{
  const sel = document.getElementById("descentAlign");
...
/* ═══ ОКНО «🪞 КРЕСТ» (v1.548) — перенесено из Zazerkalius ═══
   ...
   но только когда сменилось что-то из ключа (строка, её биты, порог, галка) — иначе на длинной
   цепочке крест пересобирался бы на каждый кадр. Строка длиннее CROSS_MAX режется для показа.
   Оси — тем же Манакером, что «🪞 Карта осей» (manacherOdd/Even, axisAntiTransform в fold-1-core),
   только здесь каждая показана скобкой: что отражается и где стоит зеркальце. */
var crossKey = "";
const CROSS_MAX = 400, CROSS_PTR_MAX = 60;
function crossTargetRow(){
...
function crossCells(s, mask){
  let h = "";
  for (let i = 0; i < s.length; i++) h += '<span class="xc b' + s[i] + (mask ? (mask[i] ? " fx" : " mv") : "") + '" data-i="' + i + '">' + s[i] + "</span>";
  return h;
}
function crossPaint(force){
  const view = document.getElementById("crossView");
  if (!view) return;
  const r = crossTargetRow();
  const s0 = r >= 0 ? st.rows[r] : "";
  const minEl = document.getElementById("crossPtrMin"), fixEl = document.getElementById("crossFix");
  const ml = Math.max(2, Math.min(64, +(minEl ? minEl.value : 4) || 4));
  const showFix = fixEl ? fixEl.checked : true;
  const key = r + "|" + ml + "|" + showFix + "|" + s0;
  if (!force && key === crossKey) return;
  crossKey = key;
  const head = document.getElementById("crossHead");
  if (r < 0) { view.innerHTML = ""; if (head) head.textContent = "нужна строка из одних 0 и 1 — выдели её"; return; }
  const s = s0.length > CROSS_MAX ? s0.slice(0, CROSS_MAX) : s0;
  const n = s.length;
  if (head) head.textContent = `строка ${typeof rowLabel === "function" ? rowLabel(r) : r} · ${s0.length} бит` + (s0.length > CROSS_MAX ? ` (показаны первые ${CROSS_MAX})` : "");
  const rev = s.split("").reverse().join("");
  const inv = s.replace(/[01]/g, c => c === "1" ? "0" : "1");
  const invRev = inv.split("").reverse().join("");
  const mask = []; let fx = 0;
  for (let i = 0; i < n; i++) { mask[i] = s[i] === s[n - 1 - i]; if (mask[i]) fx++; }
  const m = showFix ? mask : null;
  const q = (cls, lab, img) => '<div class="xq ' + cls + '"><div class="xlab">' + lab +
    (img === s && lab !== "здесь" ? ' <span class="xeq">= сама строка</span>' : "") + '</div><div class="xrow">' + crossCells(img, m) + "</div></div>";
  let h = '<div class="xgrid' + (showFix ? " showfix" : "") + '">' +
    q("l", "здесь", s) + '<div class="xv"></div>' + q("r", "⇄ разворот", rev) + '<div class="xh"></div>' +
    q("l", "🔁 инверсия", inv) + '<div class="xv"></div>' + q("r", "⇄🔁 инв-разворот", invRev) + "</div>";
  const orbit = new Set([s, rev, inv, invRev]).size;
  const stab = [];
  if (rev === s) stab.push("⇄ разворот — палиндром");
  if (invRev === s) stab.push("⇄🔁 инв-разворот — антипалиндром");
  h += `<div class="xstat">орбита <b>${orbit}</b> из 4 · ${stab.length ? "строку держит " + stab.join(" и ") : "ни одно зеркало её не держит"} · неподвижных бит <b>${fx}</b>, меняющихся <b>${n - fx}</b></div>`;
  // Указатели: все максимальные оси от ml бит, длинные — выше, по дорожкам без наложений.
  const axes = [];
  if (n >= ml) {
    const d1 = manacherOdd(s), d2 = manacherEven(s), d2a = manacherEven(axisAntiTransform(s));
    for (let c = 0; c < n; c++) {
      const l1 = 2 * d1[c] - 1;
      if (l1 >= ml) axes.push({ kind: "pal", pos2: 2 * c, lo: c - d1[c] + 1, hi: c + d1[c] - 1, len: l1 });
      const l2 = 2 * d2[c];
      if (l2 >= ml) axes.push({ kind: "pal", pos2: 2 * c - 1, lo: c - d2[c], hi: c + d2[c] - 1, len: l2 });
      const la = 2 * d2a[c];
      if (la >= ml) axes.push({ kind: "anti", pos2: 2 * c - 1, lo: c - d2a[c], hi: c + d2a[c] - 1, len: la });
    }
  }
  axes.sort((a, b) => b.len - a.len || a.lo - b.lo);
  const shown = axes.slice(0, CROSS_PTR_MAX);
  const lanes = [];
  let ph = "";
  shown.forEach((a, idx) => {
    let k = 0;
    for (;; k++) { if (!lanes[k]) lanes[k] = []; if (lanes[k].every(([l, hh]) => a.hi < l || a.lo > hh)) { lanes[k].push([a.lo, a.hi]); break; } }
    const ax = ((a.pos2 + 1) / 2 - a.lo) * 0.62;
    const where = (a.pos2 % 2 === 0) ? `по биту ${a.pos2 / 2}` : `в шве между битами ${(a.pos2 - 1) / 2} и ${(a.pos2 + 1) / 2}`;
    ph += '<div class="xptr' + (a.kind === "anti" ? " anti" : "") + '" data-lo="' + a.lo + '" data-hi="' + a.hi + '" data-kind="' + a.kind +
          '" style="left:' + (a.lo * 0.62) + "em;width:" + ((a.hi - a.lo + 1) * 0.62) + "em;top:" + (4 + k * 10) + "px;--ax:" + ax + 'em" title="' +
          (a.kind === "pal" ? "палиндром" : "антипалиндром") + ", " + a.len + " бит (" + a.lo + "…" + a.hi + "), ось " + where + '"></div>';
  });
  const np = axes.filter(a => a.kind === "pal").length;
  h += '<div class="xpbits">' + crossCells(s, null) + "</div>" +
       '<div class="xp" style="height:' + (lanes.length ? 8 + lanes.length * 10 : 0) + "px;width:" + (n * 0.62) + 'em">' + ph + "</div>" +
       '<div class="xleg">' + (axes.length
         ? `🎯 указатели: палиндромов ${np}, антипалиндромов ${axes.length - np}, самая длинная ось — ${axes[0].len} бит` +
           (axes.length > CROSS_PTR_MAX ? ` (показаны ${CROSS_PTR_MAX} длиннейших)` : "") + " · наведи на скобку — кусок подсветится"
         : `🎯 осей от ${ml} бит нет — убавь порог`) + "</div>";
  view.innerHTML = h;
  const xp = view.querySelector(".xp"), bits = view.querySelectorAll(".xpbits .xc");
  if (xp) {
    xp.onmouseover = (e) => {
      const p = e.target.closest(".xptr"); if (!p) return;
      const lo = +p.dataset.lo, hi = +p.dataset.hi, cls = p.dataset.kind === "pal" ? "hl" : "hla";
      bits.forEach((b, i) => b.classList.toggle(cls, i >= lo && i <= hi));
    };
    xp.onmouseout = () => bits.forEach(b => b.classList.remove("hl", "hla"));
  }
}
{
  const a = document.getElementById("crossFix"), b = document.getElementById("crossPtrMin");
  if (a) a.addEventListener("change", () => crossPaint(true));
  if (b) b.addEventListener("change", () => crossPaint(true));
}

/* ═══ ДОСТРОЙКА ВВЕРХ: «🔻−1» и «🔢−1» (v1.553) ═══
   Запрос пользователя: «и ещё кнопки нужны, чтоб достраивал не вниз, а вверх».
   «🔻−1» — ОБРАТНЫЙ шаг Паскаля: из строки длиной T получается та, что стояла бы НАД ней, T−1 бит.
   Считается однозначно и без перебора: у «🔺+1» крайний бит равен крайнему биту сверху, поэтому
   x[0] = y[0], дальше x[j] = y[j] ⊕ x[j−1]. Последний бит y служит проверкой: если y[T−1] ≠ x[T−2],
   такой строки над этой не существует — строка не могла получиться шагом Паскаля ни из чего
   (ровно половина всех строк такие: у «родителя» T−1 бит, значит детей вдвое меньше, чем строк).
   «🔢−1» — число минус единица, той же ширины; у нуля предшественника нет. */
const bSierpPrevEl = document.getElementById("bSierpPrev");
if (bSierpPrevEl) bSierpPrevEl.onclick = () => insertNextBelow("🔻−1", "Серпинский −1", src => {
  if (!/^[01]+$/.test(src)) return { err: "в строке не только 0 и 1 — обратный шаг считать не из чего." };
  if (src.length < 2) return { err: "строка в один бит — выше неё Паскаль уже ничего не строит." };
  let out = "";
  let prev = 0;
  for (let j = 0; j + 1 < src.length; j++) { const v = (src[j] === "1" ? 1 : 0) ^ prev; out += v; prev = v; }
  if ((src[src.length - 1] === "1" ? 1 : 0) !== prev) {
    return { err: "у этой строки нет строки-родителя: шагом «🔺+1» её получить нельзя (последний бит не сошёлся). Такова половина всех строк." };
  }
  return { out };
}, true);
const bNumPrevEl = document.getElementById("bNumPrev");
if (bNumPrevEl) bNumPrevEl.onclick = () => insertNextBelow("🔢−1", "Номер −1", src => {
  if (!/^[01]+$/.test(src)) return { err: "в строке не только 0 и 1 — это не двоичное число." };
  const v = BigInt("0b" + src);
  if (v === 0n) return { err: "это ноль — меньше не бывает." };
  return { out: (v - 1n).toString(2).padStart(src.length, "0") };
}, true);

/* ═══ СПУСК В САМУ ЦЕПОЧКУ: «⬇ Вставить» и «⊕ XOR-ом» (v1.560) ═══
   Запрос пользователя: «это тоже надо — возможность вставлять, сдвигая, или XOR-ом, вниз-вверх».
   Спуск до сих пор только показывал; теперь его можно положить в цепочку двумя способами и в две
   стороны. Что кладём — весь треугольник (этажи −1…−(n−1), то есть n−1 строк) или только левый
   край одной строкой. Вниз — этаж k встаёт на k строк ниже исходной, вверх — на k выше, поэтому
   вверх список идёт в обратном порядке: ближе всего к исходной снова этаж −1.
...
function descentPick(tag){
  ...
  let r = -1;
  for (const i of sel) { const s = st.rows[i] || ""; if (s.length && /^[01]+$/.test(s)) { r = i; break; } }
  if (r < 0) { say(`${tag}: нужна строка из одних 0 и 1 — выделите её (без точек и пустот).`); return null; }
  const levels = descentOf(st.rows[r]);
  return { r, levels, edge: levels.map(l => l[0]).join("") };
}
/* Сдвиг построчных привязок на count строк вниз, начиная с idx, — тот же перенос, что при вставке
   одной строки в insertNextBelow, только кратный. */
function descentShiftMaps(idx, count){
  for (const m of [insertedFlagsMap, invFlagsMap, newBitsMap, maskChangedMap, axisOffsetMap, axisBitShiftMap, axisBitDirMap, rowRotOffMap, mirrorsRowDone]) {
    if (!m || !m.size) continue;
    const moved = [];
    for (const [k, v] of m) moved.push([k >= idx ? k + count : k, v]);
    m.clear();
    for (const kv of moved) m.set(kv[0], kv[1]);
  }
  if (st.rowDividers && st.rowDividers.size) {
    const sh = new Set();
    for (const d of st.rowDividers) sh.add(d >= idx ? d + count : d);
    st.rowDividers = sh;
  }
  if (typeof st.aIdx === "number" && st.aIdx >= idx) st.aIdx += count;
  if (typeof st.bIdx === "number" && st.bIdx >= idx) st.bIdx += count;
}
function descentWrite(xor){
  const tag = xor ? "▽ ⊕" : "▽ ⬇";
  const pick = descentPick(tag);
  if (!pick) return;
  const whatEl = document.getElementById("descentWhat"), dirEl = document.getElementById("descentDir");
  const what = whatEl ? whatEl.value : "tri";
  const up = dirEl ? dirEl.value === "up" : false;
  ...
  if (xor) {
    ...
    for (let k = 0; k < targets.length; k++) {
      const t = targets[k], row = (t >= 0 && t < st.rows.length) ? (st.rows[t] || "") : null;
      if (row === null || !row.length) { say(`${tag}: ${up ? "выше" : "ниже"} строки ${rowLabel(r)} не хватает строк — нужно ${lines.length}, а есть ${k}. Вставь их или возьми «⬇ Вставить».`); return; }
  ...
  snapshot();
  rowCountMarkInsert();   // v1.561: то же для спуска
  const idx = up ? r : r + 1;
  const put = up ? lines.slice().reverse() : lines;
  ...
  st.hit = null;
  render(); saveCache();
  say(`${tag}: ${what === "edge" ? "край" : "треугольник"} от строки ${rowLabel(up ? r + put.length : r)} вставлен ${up ? "выше" : "ниже"} — строк ${put.length}, остальные сдвинуты. ↩ вернёт.`);
  logStep("Спуск ⬇", rowLabel(r), "", `${put.length} стр. ${up ? "вверх" : "вниз"}`);
}
{
...
/* ═══ ПОЛУОБОРОТ: ПРЫЖОК СРАЗУ В ЗАЗЕРКАЛЬЕ (v1.513) ════════════════════════════
   Запрос пользователя по разбору «Круга Инв»: «напрашивается кнопка полуоборот — прыгнуть сразу
   на n шагов, то есть мгновенно в зазеркальный лист, без прокрутки».
   ОТКУДА ОНА БЕРЁТСЯ. ◄/►Круг Инв переворачивает бит, переходящий через шов строки. Значит за
   len нажатий каждый бит пересекает шов РОВНО ОДИН раз: он возвращается на своё место, но
   перевёрнутым. То есть len тактов «Круга Инв» = инверсия строки БЕЗ всякого поворота, а ещё len
   тактов возвращают её домой. Полный период — 2·len (ровно поэтому computeShiftTotalTurns
   удваивает итог у Inv-режимов, см. fold-1-core), и на его половине строка стоит своей изнанкой.
   Кнопка делает этот прыжок одним действием и за O(len), а не за len поворотов строки.
   ПОЧЕМУ НЕ ЦИКЛ ИЗ len ВЫЗОВОВ shiftOneRowAxisAware(). Во-первых, это len копий строки по всей
   её длине — квадрат, на тысяче бит уже заметный. Во-вторых, и это важнее, у ◄/►Круг есть
   надстройки, которые ищут ПОЛОЖЕНИЕ: «⇔ Крайние «1»», «🔗 Не рвать «1»-группы», а ⊙-выравнивания
   («Ось», «Ось 1.2», «ОсьБит») вообще двигают только картинку и самих бит не трогают. Такой цикл
   они бы перехватили и увели куда угодно. Полуоборот положения НЕ ищет — он переводит биты на
   второй лист, — и потому от выравнивания и от этих галок не зависит вовсе.
   ФЛАГИ. invFlagsMap («бит перевёрнут швом») переключается у ВСЕХ бит: каждый прошёл шов по разу.
   newBitsMap («бит новый») не трогаем — у Inv-поворотов эта пометка ездит ОБЫЧНЫМ поворотом (см.
   NEW_BITS_ROT в fold-1-core), а len обычных поворотов возвращают её ровно на место.
   СЧЁТЧИК «Вар: N/M». Длины одинаковые — полуоборот всего набора это ровно len тактов, на len
   счётчик и двигается. Длины разные — общего числа тактов у прыжка нет, у каждой строки свой
   полуоборот; берём наибольшую длину (N не убежит за M, ведь M = 2·НОК длин ≥ 2·max), а в
   сообщении честно говорим, что каждая строка прыгнула на свою величину. */
/* v1.640, «Полуоборот — как галка, запоминать последнее нажатие (как Круг)… и работать в шаге; если несколько строк выделено —
   нижняя выделенная делает свой полуоборот, а верхние — этот же её полуоборот своей длины, то есть больше сделают: строка в
   2 раза уже сделает полный оборот»; следом «и в другую сторону также». ПРАВИЛО: k = длина НИЖНЕЙ выделенной строки, и КАЖДАЯ
   выделенная строка делает k тактов ИнвКруга в свою сторону. У строки длины L это m = k mod 2L тактов: q = m mod L бит уходят
   через шов (перевёрнутыми), а если m ≥ L — вся строка ещё и стоит изнанкой. Нижняя: m = L — чистая изнанка, как было. На бит
   короче: m = L + 1 — изнанка и сдвиг на бит. Вдвое короче: m = 2L — полный оборот, строка дома. Одна выделенная строка — то же,
...
/* v1.651, «также Реверс — подключи к Авто, Шагу». «⇄ Реверс» в группе «Круг» — запоминаемый режим, как кнопки Круга: нажатие
   разворачивает выделенные строки (ничего не выделено — все) и подсвечивается, а «▶ Шаг», «🚀 Авто» и «🔬 Анализ» повторяют разворот —
   с поиском, захватом, логом и счётчиком. Цикл — 2 хода: второй разворот возвращает строки (строки-палиндромы от разворота не
   меняются вовсе — если все такие, ход один). Пометки «перевёрнут» и «новый» едут вместе со своими битами — массивы разворачиваются
   тем же ходом. Старая «⇄ Реверс» в «Строках» (st.revToggled) осталась как была. */
function reverseRows(idxs){
  for (const r of idxs) {
    const s = st.rows[r]; if (!s) continue;
    const L = s.length, f = getInvFlags(r, L), nb = newBitsMap.get(r);
    st.rows[r] = reverseStr(s);
    invFlagsMap.set(r, f.slice().reverse());
    if (nb && nb.length === L) newBitsMap.set(r, nb.slice().reverse());
  }
}
function reverseTotal(idxs){
  let any = false, live = false;
  for (const r of idxs) { const s = st.rows[r]; if (!s) continue; live = true; if (s !== reverseStr(s)) any = true; }
  return !live ? 0 : (any ? 2 : 1);
}
// Предел вариантов для запомненного режима Круга — один на «Авто», ручные нажатия и пересчёт после захвата.
...
if (bKrugReverseEl) bKrugReverseEl.onclick = () => {
  ...
  if (!idxs.some(r => st.rows[r])) { say("⇄ Реверс: разворачивать нечего — нет непустых строк."); return; }
  setLastDirMode("reverse");
  mirrorsBeforeShift();
  snapshot();
  reverseRows(idxs);
  afterShiftBgCheck(false);
  say(`⇄ Реверс: развёрнуто строк — ${idxs.filter(r => st.rows[r]).length}. Запомнен для «▶ Шаг» / «🚀 Авто» / «🔬 Анализ»; ещё раз — обратно.`);
  logStep("⇄ Реверс", idxs.filter(r => st.rows[r]).map(r => r + 1).join(", "), "", "Разворот выделенных строк");
  render(); saveCache();
};
