...
if (bShiftHalfTurnEl) {
  bShiftHalfTurnEl.onclick = () => {
    if (!shiftAllowed()) return; // 🔒 замок — Круг бит не двигает (см. shiftAllowed в fold-1-core)
    if (!st.selectedRows || st.selectedRows.size === 0) { say("Выделите строку кликом."); return; }
    // Режим Inv-семейства: от него зависит общее число вариантов в счётчике (2·НОК длин).
    setLastDirMode("shiftRInv");
    mirrorsBeforeShift();
    snapshot();
    const lens = [];
    for (const r of st.selectedRows) {
      const s = st.rows[r];
      const len = s ? s.length : 0;
      if (!len) continue;
      lens.push(len);
      invFlagsMap.set(r, getInvFlags(r, len).map(f => !f));
      st.rows[r] = invertBits(s);
    }
    if (!lens.length) { say("Полуоборот: в выделении нет ни одной непустой строки."); return; }
    const same = lens.every(l => l === lens[0]);
    const step = same ? lens[0] : Math.max.apply(null, lens);
    // afterShiftBgCheck() добавит к счётчику ещё один такт — остальные докладываем здесь.
    st.manualShiftTurns = (st.manualShiftTurns || 0) + step - 1;
    afterShiftBgCheck(true);
    const uniq = Array.from(new Set(lens)).sort((a, b) => a - b).join("/");
    say(same
      ? `Полуоборот: ${lens.length} стр. по ${step} бит — прыжок на ${step} тактов «Круга Инв», строки стоят изнанкой. Ещё один полуоборот вернёт их домой.`
      : `Полуоборот: строки разной длины (${uniq}) — каждая прыгнула на свою и стоит изнанкой. Счётчик вариантов двинут на ${step}.`);
    logStep("Полуоборот", Array.from(st.selectedRows).map(r => r + 1).join(", "), "", `Инверсия ${lens.length} стр.: ${step} тактов «Круга Инв» одним прыжком`);
    render(); saveCache();
  };
}
