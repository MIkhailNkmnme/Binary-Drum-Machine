...
/* ЦИФРЫ НАЛОЖЕНИЯ С РАЗДЕЛЬНЫМ ЦВЕТОМ «1» И «0» (v1.111, запрос пользователя: "для наложения надо
   настройки цвета 0 и 1"). Красить посимвольно было бы расточительно — блок бывает в сотни бит, а
   render() зовут на каждый шаг: заворачиваем СЕРИЯМИ одинаковых цифр, так на «111000» уходит два
...
function resetAll(){
  ...
  /* СБРОС УБИРАЕТ ПУСТОЙ ДОБОР СВЕРХУ (v1.157, баг-репорт: «откуда-то взялась достройка вверх
     после Escape»).
     Виноват авторост полотна (v1.146): когда блок уводят выше нулевой строки, сверху добавляются
     пустые строки, и чтобы номера существующих не поехали, счётчик st.topBuilt растёт вместе с
     ними. Побочно эти строки становятся неотличимы от построений вкладки «Вверх» — а те Сброс
     сохраняет намеренно. Отсюда и «достройка», которой человек не делал.
     Отличить их просто: настоящие построения — это зеркала, они НЕПУСТЫЕ. Поэтому снимаем сверху
     ровно те строки, которые пусты и по битам, и по паттерну, и под которыми нет наложения: они
     заведомо наши, доборные. Первая же непустая — стоп, дальше чужое.
     ...
     которым авторост двигал их вперёд. */
  {
    ...
    if (pad > 0) {
      ...
      st.pats.splice(0, pad);
      st.topBuilt = Math.max(0, (st.topBuilt || 0) - pad);
      if (st.selectedRows && st.selectedRows.size) st.selectedRows = new Set(Array.from(st.selectedRows).map(r => r - pad).filter(r => r >= 0));
      if (st.selectedPats && st.selectedPats.size) st.selectedPats = new Set(Array.from(st.selectedPats).map(r => r - pad).filter(r => r >= 0));
      if (st.rowDividers && st.rowDividers.size) st.rowDividers = new Set(Array.from(st.rowDividers).map(d => d - pad).filter(d => d >= 0));
      for (const key of ["rowGroups", "rowGroupsL", "rowGroupsR"]) {
        for (const g of (st[key] || [])) g.rows = g.rows.map(r => r - pad).filter(r => r >= 0);
      }
      if (typeof rowGroupsTouch === "function") rowGroupsTouch();
      st.aIdx = Math.max(0, (st.aIdx | 0) - pad);
      st.bIdx = Math.max(0, (st.bIdx | 0) - pad);
      if ((st.horizonRow | 0) > 0) st.horizonRow = Math.max(0, (st.horizonRow | 0) - pad);
      for (const pp of (Array.isArray(st.pastes) ? st.pastes : [])) if (pp) pp.row = (pp.row | 0) - pad;
  ...
  /* СБРОС СНИМАЕТ ПОМЕТКУ ПОИСКА У ВСЕХ ПАТТЕРНОВ (испр. v1.151, баг-репорт: «сброс снова не
     ...
     ТОЛЬКО те, что пересобираются. Строки, достроенные СВЕРХУ (keepTop/topBuilt), возвращаются на
     место прежними объектами, вместе со своими found/kind/step, и зелёная пометка на них Сброс
     переживала. Заметно это стало тогда, когда строк сверху стало много (v1.141/1.146 — рост
  ...
  /* АВТОВЫДЕЛЕНИЕ ПЕРВОЙ СТРОКИ ПРИ СБРОСЕ УБРАНО (v1.150, баг-репорт «при сбросе не надо выделять
     первую строку»). Раньше, если после Сброса выделения не оставалось, выделялась первая строка с
     данными — по прежней просьбе «пусть всегда одна выделена». Смысл у этого был такой: без
     выделения Круг и Спираль крутят ВСЕ строки, а сквозные склейки не стартуют вовсе, и на пустом
     выделении это выглядело необъяснимо.
     Сейчас цель есть и без выделения — её даёт горизонт (см. computeBgSearchTarget/seqAnchorIdx,
     v1.149), — а навязанное выделение мешает: Сброс должен возвращать чистое состояние, а не
     выбирать что-то за человека. Само выделение, если оно БЫЛО до Сброса, по-прежнему возвращается
     строкой выше (keepSel). */
...
function renderTabs() {
  ...
  '<div class="chain-dd-footer">' +
    ...
    '<button type="button" class="chain-dd-file" id="bDdExport" data-act="export" title="Выгрузить в файл .json ВСЁ, что приложение помнит: вкладки-цепочки со строками и паттернами, настройки вида/поиска, сохранённый снимок настроек и раскладки панелей, кэш паттернов и слоты цветов (v1.453)">⬇ Экспорт</button>' +
    '<button type="button" class="chain-dd-file" id="bDdImport" data-act="import" title="Загрузить из файла .json вкладки-цепочки (заменит все текущие), а если файл выгружен с v1.453 и новее — ещё и настройки вида/поиска, снимок с раскладкой панелей, кэш паттернов и слоты цветов">⬆ Импорт</button>' +
    '<button type="button" class="chain-dd-file chain-dd-wipe" id="bDdWipe" data-act="wipe" title="ПОЛНОСТЬЮ очистить кэш приложения в браузере: все вкладки-цепочки, настройки и раскладку панелей. Нужно, когда сохранённое состояние само вешает вкладку. Сначала выгрузи нужное кнопкой «⬇ Экспорт»">🗑 Кэш</button>' +
    '</div>' +
  // Выгрузка ТЕКУЩЕЙ цепочки в Excel (v0.966, запрос пользователя) — своим рядом: это не дамп
  // состояния, как «⬇ Экспорт», а картинка для печати/разбора, и формат у неё совсем другой
  // (см. exportChainToExcelXml).
    '<div class="chain-dd-frow">' +
    '<button type="button" class="chain-dd-file" id="bDdXlsx" data-act="xlsx" title="Выгрузить ТЕКУЩУЮ цепочку в Excel (формат «Таблица XML 2003», открывается двойным кликом). Столбец A — вся строка одним текстом, столбец B пустой, дальше с C — ПО ОДНОМУ БИТУ В ЯЧЕЙКУ, ровно с тем же выравниванием, что на экране. Формат ячеек текстовый, ячейки мелкими квадратиками под шрифт 8, цвета бит и фон полотна переносятся как есть. «½»-выравнивания встают в ближайший целый столбец — половины столбца в Excel не бывает">📊 Excel XML (цепочка)</button>' +
    '</div>' +
  ...
  // Счётчик в подписи — сколько записей сейчас в кэше; пустой кэш обе кнопки отключает.
    '<div class="chain-dd-frow">' +
    '<button type="button" class="chain-dd-file chain-dd-bank" id="bDdBankSave" data-act="banksave" title="Отложить в кэш ТЕКУЩИЕ паттерны этой цепочки — отдельной копией. Дальше их можно как угодно править, переключать цепочки и сбрасывать: отложенное в кэше не меняется, пока не отложишь заново. Лежит в браузере, переживает перезагрузку">💾 Паттерны в кэш' + (patBankCount ? ' (' + patBankCount + ')' : '') + '</button>' +
    '</div>' +
    '<div class="chain-dd-frow">' +
  ...
  /* ═══ ID У КНОПОК ПОДВАЛА — РАДИ СЛОТОВ, А НЕ РАДИ ОБРАБОТЧИКОВ (v1.292) ═══
     Баг-репорт пользователя: «кнопка „Сохр. настройки“ не копируется в слоты для кнопок… я про эти
     кнопки — они не перетаскиваются все».
     Механизм закрепления опознаёт кнопку ТОЛЬКО по id: `closest("button[id]")` в mousedown/dragstart
     и `getElementById` при отрисовке ярлыка (см. fold-5-ui.js). Весь подвал был на классах и
     data-act, поэтому ни одна его кнопка перетаскиванию не поддавалась — и это ощущалось как
     поломка, хотя было границей реализации.
     Раздаём id. Сказанное выше при этом в силе и не отменяется: обработчик по-прежнему
     делегированный, по data-act. id тут — не точка подвески, а ИМЯ, по которому слот потом находит
     кнопку заново; сам узел renderTabs() пересоздаёт при каждой перерисовке.
     Дублей не будет: подвал рисуется один на весь список, в отличие от построчных .chain-dd-mini
     (data-act="save"/"reset" с data-idx) — тем id не даём намеренно, их по одной паре на вкладку. */
  ...
  // (см. copySelectedRows/clipRowsHtml).
    '<div class="chain-dd-frow">' +
    '<button type="button" class="chain-dd-file chain-dd-ui" id="bDdCopyChain" data-act="copychain" title="Скопировать строки цепочки в буфер обмена — каждая своей строкой, с ведущими пробелами по текущему выравниванию. Выделены строки — копируются только они, ничего не выделено — вся цепочка. В Excel вставляется с цветами символов (строка = одна ячейка). То же самое делает Ctrl+C по выделенным строкам">📋 Скопировать строки-цепочки</button>' +
    '</div>' +
    '<div class="chain-dd-frow">' +
    '<button type="button" class="chain-dd-file chain-dd-ui" id="bDdUiSave" data-act="uisave" title="Сохранить текущие настройки вида/поиска (цвета, шрифт, галки и т.п.) И раскладку панелей — где какая закреплена и где стоят плавающие. Данные цепочек не трогает">💾 Сохр. настройки</button>' +
    '<button type="button" class="chain-dd-file chain-dd-ui" id="bDdUiReset" data-act="uireset" title="Вернуть настройки вида/поиска к СОХРАНЁННЫМ — тем, что записала кнопка «💾 Сохр. настройки» (v1.451). Своего снимка ещё нет — тогда к значениям по умолчанию. Данные цепочек не трогает">↺ Сброс настроек</button>' +
    '</div>' +
  '</div>';
}
...
function renderColHeader(maxLen, mirrorPadL){
  ...
  /* ОБЩИЙ БАЛАНС В ПОЛОСЕ — ТОЛЬКО ПОКА ВКЛЮЧЕНЫ ПОСТРОЧНЫЕ БАЛАНСЫ (v0.864: "всегда на экране";
     сужено в v0.995, запрос пользователя: "показывай только при включении балансов" — заодно и
     переезд в «Лог находок» из v0.993 отменён тем же заходом, "не туда направил тебя": элемент
     вернулся в полосу выравниваний, рядом со строками, которые он же и подписывает).
     Включаются построчные балансы кнопкой «⚖ Балансы» во вкладке «Вид» (#bShowBalances) — ОНА
     остаётся независимым мастер-переключателем и работает всегда, даже когда эта полоска скрыта.
     Пока полоска ВИДНА, обе её половинки (обычные числа / они же в двоичном) — ещё и пара кнопок:
     клик переключает ВИД построчных балансов, повторный клик по уже активной — выключает их
     совсем (см. обработчик #alignBalance в fold-3). */
  if (alignBalEl) {
    if (!st.showBalances) {
      alignBalEl.style.display = "none";
    } else {
      ...
      const decAct = !st.binBalance ? " act" : "";
      const binAct = st.binBalance ? " act" : "";
  ...
  el.innerHTML =
    /* ═══ РАСПОРКИ .pat-dot — И ЗДЕСЬ ТОЖЕ (v1.330) ═══
       В строке колонки идут .pat → .pat-dot → .num-l2 → .bits → .pat-dot → .pat2 (см. render()),
       а в этой линейке точек не было вовсе. Пока они были обычной колонкой в 10px, линейка ехала
       относительно строк на зазор — тот самый рассинхрон, который в fold-5-ui.js называли
       поимённо (v1.313: «в линейке нет .pat-dot»), и который каждый раз обходили, меряя поле по
       строке, а не по линейке.
       С v1.330 колонка схлопнута в ноль и тянет себя назад отрицательными полями (см. .pat-dot в
       стилях), то есть её вклад в раскладку стал ОТРИЦАТЕЛЬНЫМ: −2 × --row-gap. Разница между
       линейкой и строкой выросла бы вдвое и уже бросалась бы в глаза на засечках.
       Ставим те же две распорки — пустые, без знака: структура колонок у линейки и у строки теперь
       совпадает элемент в элемент, и разъехаться им нечем. Обходные пути в fold-5-ui.js не трогаю:
       они меряют поле по строке и от этого остаются верными. */
    '<div class="col-hdr-row"><span class="pat"></span><span class="pat-dot"></span>' +
    '<span class="num num-l2"></span>' +
    '<span class="bits ' + alignCls + '"><span' + offAttr + '>' + ticks + '</span></span>' +
    '<span class="pat-dot"></span><span class="pat2"></span></div>';
}
...
/* ═══ ПЛАНКИ ПОЛЕЙ ЕДУТ ВМЕСТЕ С ГОРИЗОНТАЛЬНОЙ ПРОКРУТКОЙ (испр. v1.376) ═══
   Баг-репорт: «при гориз. скролле меню 3 не едет вместе со своим полем, только при клике по полю
   встают на место».
   Планки («П1», «П2», «№») и полоса выравниваний стоят абсолютом, а место им считает
   updateSplitPositions() по ЖИВЫМ коробкам колонок. Пока цепочка помещается в холст, всё сходится;
   стоит ей стать шире и поехать по горизонтали — коробки уезжают, а планки остаются на числах,
   посчитанных до прокрутки. Клик по полю просто вызывал перерисовку, и та возвращала их на место —
   отсюда и «встают только при клике».
   Вешаем пересчёт прямо на прокрутку. Отдельным слушателем, а не внутри vrowsOnScroll: та занята
   вертикальной виртуализацией и может выйти рано (окно строк не изменилось), а планки надо
   двигать на любой сдвиг, в том числе чисто горизонтальный.
   Через requestAnimationFrame: событие прокрутки приходит десятками в секунду, а пересчёт меряет
   DOM — без склейки кадров он бы тормозил саму прокрутку. Флаг не даёт накопить очередь заявок.
   passive:true — прокрутку не отменяем, только смотрим. */
(function bindStripsToScroll(){
  const sc = document.getElementById("screenCanvas");
  if (!sc) return;
  let pending = false;
  sc.addEventListener("scroll", () => {
    ...
    requestAnimationFrame(() => {
      ...
      /* ═══ И ПОЛОСА ВЫРАВНИВАНИЙ ТОЖЕ (испр. v1.376 → v1.377) ═══
         Баг-репорт: «ось не отстаёт, а меню по-прежнему».
         Место полосы считается НЕ в updateSplitPositions, как я решил, а в самом конце
         updateAxisSplitPosition — там, где уже посчитана коробка поля бит, по которой полоса
         центруется. Мой хук звал только первую, поэтому планки поехали, а полоса осталась.
         Зовём и вторую. maxLen ей нужен один — длина самой длинной строки: без него она просто
         прячет ось (см. её первую проверку). Считаем по данным, как и везде: при виртуализации
         самой длинной строки может не быть в разметке.
         Ось при этом и не отставала — её координата чисто .chain-относительная и от прокрутки не
         зависит; лишний пересчёт ей не вредит, а разводить два вызова ради этого незачем. */
      if (typeof updateAxisSplitPosition === "function") {
        let mx = 0;
        for (const s of (st.rows || [])) { const n = s ? s.length : 0; if (n > mx) mx = n; }
        updateAxisSplitPosition(mx);
      }
    });
  }, { passive: true });
})();

/* Собственные поля ячейки П1 (padding-left + padding-right у .pat): слева ноль, справа резерв под
   бейдж «#N». Меряем с живой ячейки — стили задают их в одном месте, и повторять число здесь
   нельзя (см. разбор у colPx в render, v1.442). Ячеек ещё нет — отдаём то, что стоит в CSS. */
function patCellPadPx(){
...
function render(){
  ...
  /* ГРУППЫ И У КРАЙНИХ ПОЛЕЙ ТОЖЕ (v0.982). Та же механика, что у цепочки (см. rowAlignCtx в
     fold-1-core.js): выделенные строки получают своё выравнивание, а рамкой работает НИЖНЯЯ из
  ...
  /* Ширина столбца — ещё и в CSS (v1.447): стилям она нужна там, где отступ надо задать В БИТАХ, а
     не в пикселях (см. padding-right у .pat при выравнивании вправо). Пишем ту же величину, что
     считает разметку, чтобы второго определения «ширины бита» не завелось. */
  if (colStepPx > 0) document.documentElement.style.setProperty("--col-step", colStepPx.toFixed(3) + "px");
  ...
  bgInfoLast = bgInfo;
  /* НАЙДЕННОЕ ЗАКРЕПЛЯЕТСЯ ДО СБРОСА (v1.156, запрос: «красит жёлтым найденную, потом перемещаю
     наложение — сквозная пересчиталась и уже другая, но то, что нашёл, надо зафиксировать зелёным…
     до сброса»). Жёлтая пометка фон-поиска живёт РОВНО ОДИН кадр: сдвинул блок — сквозная другая,
     совпадения в ней нет, и находка пропадает бесследно. Поэтому на каждый успешный поиск ставим
     паттерну стойкий признак bgFound: он не зависит от текущей сквозной и снимается только Сбросом
  ...
  /* Нулевая строка из набора выделенных вычищается на каждой отрисовке (v1.407). Выделять её
     запретили в v1.406, но попасть в набор она могла раньше — и приехать оттуда с сохранённой
     цепочкой или из кэша вида. Гасить это только стилями мало: на выделение смотрят Круг,
     построения и «Авто», и невидимая нулевая в наборе тихо меняла бы их поведение.
  ...
  for (let i = vr.lo; i <= vr.hi; i++){
    ...
    /* ═══ ПРИЗНАК НУЛЕВОЙ СТРОКИ — ПО НОМЕРУ, А НЕ ПО ИНДЕКСУ (испр. v1.407 → v1.408) ═══
       Баг-репорт с разметкой в руках: у строки, которую видно как нулевую, data-idx оказался 61.
       И правильно: индекс — это место в массиве, считая от самого верха вместе с полем наложений,
       а «нулевая» — та, чей НОМЕР равен нулю, то есть первая строка собственно цепочки. Пока поле
       наложений закрыто, они совпадают; открыл его — и расходятся ровно на его высоту.
       В v1.407 я повесил стили на data-idx="0" и промахнулся мимо на всю высоту поля. Даём строке
       честный класс, считая номер тем же rowLabel, которым он и печатается, — и стили, и любые
       будущие правила смогут опираться на него, а не угадывать индекс. */
    if (typeof rowLabel === "function" && rowLabel(i) === 0) cls.push("row-zero");
    ...
    /* ═══ НЕТ СТРОКИ В БАЗЕ — ЗНАЧИТ ОНА НОВАЯ, А НЕ «СОВПАЛА» (испр. v1.318) ═══
       Баг-репорт: «поправь раскраску номеров 1 строки — там почему-то биты номеров паттернов в
       этой строке не красятся синим». Гаснут при этом ОБА номера строки (и в колонке цепочки, и
       внутри ячейки паттерна): класс считается один на строку, вот он.
       ЧТО БЫЛО НЕ ТАК. `changedBase[i] || ""` сваливало в одно два разных ответа: «в базе эта
       строка пустая» и «в базе такой строки НЕТ ВОВСЕ» (i за пределами массива — undefined).
       Второе выдавалось за первое, и пустая строка цепочки совпадала с этой выдумкой.
       ГДЕ ВЫЛЕЗАЛО. На нулевой строке. Она всегда пуста по построению (см. ensureZeroRow в
       fold-4-tools.js), а её ИНДЕКС — это st.topBuilt, то есть он уезжает вниз с каждой
       достройкой вверх. Настроил десяток строк над цепочкой — и индекс нулевой ушёл далеко за
       конец шаблона: базы там нет, сравнивать не с чем, но старая формула честно докладывала
       «совпало». Отсюда и картинка: синие номера у всех строк подряд и один серый посередине,
       ровно на нулевой, — без всякой видимой причины.
       КАК ТЕПЕРЬ. Строки нет в базе — значит «↺ Сброс» её не вернёт, она новая: это изменение, и
       номер синий. Пустая строка, которая в базе ТОЖЕ пуста, по-прежнему считается неизменённой —
       такое сравнение осмысленно и осталось как было. */
    const baseRow = changedBase[i];
    /* ОТРИЦАТЕЛЬНЫЙ НОМЕР — КЛАССОМ, А НЕ ЗНАКОМ (v1.323, запрос «не печатай минус-знак,
       но имей ввиду его»). Строки выше нулевой (достроенные сверху, st.topBuilt) нумеруются в минус,
       и теперь об этом говорит цвет номера (--num-neg-c, см. .num.neg в fold.html). Класс общий
       на все три номера строки — колонка поля (.num-l2) и номера внутри ячеек П1/П2 (.num-p1/.num-r2):
       число у них одно и то же, значит и признак один. */
    const numCls = "num" + (baseRow === undefined || s !== baseRow ? " changed" : "")
                 + (rowLabel(i) < 0 ? " neg" : "");
    ...
    /* ═══ ГЕОМЕТРИЯ ЯЧЕЙКИ — ОДНА НА ОБЕ ВЕТКИ, И С ПАТТЕРНОМ, И БЕЗ (v1.322) ═══
       Баг-репорт пользователя: «0 номер паттернов уехал вправо» — то есть номера всех строк
       встали по выравниванию, а номер строки С ПУСТЫМ паттерном остался у левого края
       ячейки, в стороне от остальных.
       Причина: весь расчёт отступа (padL/padR, полушаги, поправка на переполнение)
       лежал ВНУТРИ ветки «у строки есть паттерн», а пустая ветка печатала голый номер
       без единого пробела отступа. Переносим расчёт НАРУЖУ: длина паттерна у пустой
       строки просто ноль, и та же формула ставит её номер туда же, где кончаются паттерны
       соседних строк. Копии формулы для второй ветки сознательно не заводим — разъедется. */
    const patLen = (p && p.text) ? p.text.length : 0;
    ...
    const padL = patFieldPad(patAlign, patLen, i, "L");
    /* padR считается НИЖЕ (v1.346) — из него вычитается ширина номера П2, а та читается из
       --num-w вместе с остальными столбцовыми величинами. */
    /* ОБЁРТКА .pat-shift (v0.976) — за неё поле двигают мышью: на ней висит
       position:relative + left:var(--pat-off-l/r), то есть ВЕСЬ напечатанный ряд едет как одно
    ...
    const halfL = patFieldHalfFor(patAlign, patLen, i, "L", maxPatLen);
    const halfR = patFieldHalfFor(pat2Align, patLen, i, "R", maxPatLen);
    ...
    /* ═══ НОМЕР В ЯЧЕЙКЕ П1 ТОЖЕ ЗАНИМАЕТ СТОЛБЦЫ (v1.322) ═══
       Запрос пользователя: «когда кнопкой создаются номера паттернов в П1 — пусть сдвигает
       биты влево, чтобы сразу было видно всё; сейчас там выравнивание вправо, номера
       создаются и их не видно, пока вручную не изменить ширину поля».
       Поправка на переполнение (v1.194, выше) считала только САМ паттерн, а номер у П1
       печатается ПОСЛЕ него (см. сборку pat ниже). При выравнивании вправо паттерн и так
       кончается у правого края колонки, значит номер целиком выходил за неё и срезался
       overflow:hidden — кнопка «№» на вид не делала ничего, пока колонку не расширяли рукой.
       Теперь в поправку входит и номер: весь ряд едет влево ровно настолько, насколько
       выступает, и номер виден сразу по клику, без правки ширины.
       ШИРИНУ БЕРЁМ ОДНУ НА ВСЕ СТРОКИ (--num-w, самый широкий номер), а не длину
       номера этой строки: иначе строка с «9» и строка с «10» съехали бы на разное число
       столбцов, и колонка стала бы рваной. Та же переменная и по тому же условию, что
       в fitPatW() и в --rownum-ins (v1.319), — резерв и сдвиг должны считаться одинаково.
       П2 сюда не входит: там номер печатается ДО отступа, у левого края своей ячейки,
       и за правый край его никакая длина паттерна не выталкивает. */
    /* ═══ ШИРИНУ КОЛОНКИ БЕРЁМ ВИДИМУЮ, А НЕ --pat-w (испр. v1.351 → v1.352) ═══
       Баг-репорт: «там ещё номера к паттернам — их не учитывает, похоже, или не сдвигает биты так,
       чтобы их было видно».
       Ширина колонки тут нужна затем, чтобы понять, НА СКОЛЬКО содержимое вылезает за её правый
       край, и на столько же увести весь ряд влево (правило v1.194, с v1.322 вместе с номером).
       Но с v1.342 колонка П1 ОТДАЁТ часть своей ширины: у .pat вычтены постоянный слот колонки
       номеров цепочки, зазор строки, а при включённых номерах в П1 — ещё и их слот. То есть
       --pat-w давно перестала быть тем, что видно на экране, и была ЗАВЫШЕНА ровно на эти
       слагаемые. Поправка их не знала, считала, что место ещё есть, и ряд не двигала — номер
       честно печатался за правым краем, где его срезает overflow:hidden. Отсюда «номера не видно,
       пока не расширишь колонку рукой» — та же жалоба, что чинилась в v1.322, и вернулась она из-за
       моей же правки ширины.
       Вычитаем ТЕ ЖЕ величины и по ТЕМ ЖЕ условиям, что и правило .pat в стилях: зазор строки —
       безусловно, слот номера в ячейке — только при st.patNumL. Разойтись им нельзя, иначе поправка
       снова начнёт врать. */
    const rootCS = getComputedStyle(document.documentElement);
    /* ═══ И ПОЛОСА ГРАНИЦЫ ТОЖЕ НЕ НАША ШИРИНА (v1.372) ═══
       Запрос пользователя: «при вкл номеров П1 двигай выровненные биты влево, чтобы было видно
       приставленные номера».
       Сдвиг влево делает поправка ниже, и с v1.361 она знаковая. Не хватало последнего слагаемого:
       правый край колонки считался по стыку полей, а на стыке с v1.354 лежит полоса границы, и её
       ПРАВАЯ половина заходит в цепочку, ЛЕВАЯ — сюда, в П1. Номер, вставший впритык к правому
       краю, оказывался ровно под этой половиной — на глаз «номера не видно», хотя обрезкой его не
       трогало.
       Вычитаем полполосы: видимая ширина колонки кончается там, где полоса начинается, и номер
       останавливается перед ней. Половина, а не вся: вторая половина полосы лежит уже в цепочке и к
       ширине П1 отношения не имеет — та же арифметика, что у заливок в v1.364. */
    const colPx = Math.max(0,
        (parseFloat(rootCS.getPropertyValue("--pat-w")) || 0)
      - (parseFloat(rootCS.getPropertyValue("--numl-slot")) || 0)
      - (parseFloat(rootCS.getPropertyValue("--row-gap")) || 8)
      - (parseFloat(rootCS.getPropertyValue("--split-w")) || 5) / 2
      /* ═══ РЕЗЕРВ ПОД НОМЕР ВЫЧИТАЕТСЯ ВСЕГДА (испр. v1.421) ═══
         Баг-репорт: «№ внутри П1, когда их нет, неверно выравнивает по краям символы».
         В v1.389 вычет под номер в ячейке сделали БЕЗУСЛОВНЫМ — иначе включение номеров двигало всю
         раскладку (разбор там же, у .pat в стилях). А здесь остался прежний вопрос «включены ли
         номера», и при выключенных две формулы разошлись: CSS отдаёт колонке на --num-slot меньше,
         чем думает этот расчёт. Поправка на переполнение считала, что места ещё хватает, и не
         сдвигала ряд — правый край паттерна уходил под обрез, а на глаз это читалось как сбитое
         выравнивание.
         Условие снято: обе стороны снова читают одну и ту же величину по одному и тому же правилу.
         Сколько столбцов занимает сам номер (numColsL ниже) по-прежнему зависит от st.patNumL, и
         это верно: место резервируется всегда, а занимает его номер, только когда он показан. */
      - (parseFloat(rootCS.getPropertyValue("--num-slot")) || 0)
      /* ═══ СОБСТВЕННЫЕ ПОЛЯ ЯЧЕЙКИ — ТОЖЕ НЕ МЕСТО ПОД ЗНАКИ (испр. v1.442) ═══
         Баг-репорт: «баг у номеров П1, когда отключены номера Ц — их не видно при выравнивании
         вправо, за границу уходят».
         У .pat справа padding в 28px — резерв под бейдж «#N», который рисуется абсолютом у самого
         края ячейки. Знаки в этот резерв не заходят: текст кончается там, где кончается
         СОДЕРЖИМОЕ, на 28px раньше границы. А поправка выравнивания (v1.361) сводила правый край
         содержимого с правым краем ВСЕЙ колонки и потому уводила ряд правее, чем он может стоять:
         хвост — а при включённых номерах в П1 это как раз номер — уезжал под бейдж и под полосу
         границы, где его срезает overflow:hidden.
         Раньше это пряталось за другим слагаемым: при включённых номерах цепочки из ширины
         вычитается ещё и их слот (--numl-slot, три-четыре десятка пикселей), и промах в 28px тонул
         в нём. Выключили номера Ц — слот обнулился, и промах вылез на глаз, ровно как в репорте.
         Вычитаем оба поля ячейки. Величину берём с живой ячейки, а не константой: padding задан в
         стилях в одном месте, и дублировать его числом здесь значило бы завести второй источник
         правды. Ячеек ещё нет (первый кадр) — берём то же, что стоит в CSS. */
      - patCellPadPx());
    const colCols = (colPx > 0 && colStepPx > 0) ? (colPx / colStepPx) : 0;
    const numColsL = (st.patNumL && colStepPx > 0)
      ? ((parseFloat(rootCS.getPropertyValue("--num-slot")) || 0) / colStepPx) : 0;
    /* ═══ ПОПРАВКА РАБОТАЕТ В ОБЕ СТОРОНЫ (v1.361) ═══
       Запрос пользователя: «надо ещё биты сдвигать, если пустота образуется, то есть располагать
       биты по своему выравниванию, но при этом ещё и учесть символы номеров и сдвинуть всё так,
       чтобы всё было видно».
       Выравнивание раскладывает паттерны по СЕТКЕ — от самого длинного паттерна (maxPatLen), а не
       по фактической ширине колонки. Пока эти две величины совпадали, разницы не было. Теперь не
       совпадают: автоширина держит запас и под номер в ячейке, и под то, что колонка отдаёт
       цепочке (см. fitPatW), и при выравнивании ВПРАВО правый край сетки оказывается ЛЕВЕЕ правого
       края колонки — между паттерном и границей открывается пустота, а номер за ним висит в ней же.
       Поправка на переполнение (v1.194) умела только одно направление: вылез — уводим влево.
       Делаем её знаковой. Считаем правый край всего содержимого ряда (отступ + паттерн + номер) и
       сводим его с правым краем колонки:
         вылезло  → сдвиг влево, как и раньше, чтобы ничего не срезалось;
         не достаёт → сдвиг вправо, ровно на недостачу, чтобы пустота ушла.
       ВТОРОЕ НАПРАВЛЕНИЕ — ТОЛЬКО ДЛЯ ПРИЖАТЫХ ВПРАВО. У выравнивания влево пустота справа не
       ошибка, а его смысл: паттерн обязан начинаться от левого края. У «по центру» уводим на
       ПОЛОВИНУ недостачи — центр считается по сетке, а должен по колонке, и половина разницы это
       ровно и есть. Лесенки не трогаем: у них сдвиг свой на каждую строку, и подгонять его по
       одной колонке значило бы сломать саму ступеньку.
       Знак: halfAttr вычитает overL из сдвига (см. её формулу ниже), поэтому «уводим вправо» — это
       отрицательный overL. */
    const shL = patFieldShiftFor(patAlign, patLen, i, "L", maxPatLen);
    const contentEndL = shL + patLen + numColsL;
    let overL = 0;
    if (colCols > 0) {
      const diffL = contentEndL - colCols;
      if (diffL > 0) overL = diffL;                                   // вылезло — влево
      else if (patAlign === "right") overL = diffL;                   // пусто справа — вправо
      else if (patAlign === "center" || patAlign === "halfcenter") overL = diffL / 2;
    }
    /* ═══ НОМЕР П2 ПРИЛИПАЕТ К ПАТТЕРНУ ПРИ ЛЮБОМ ВЫРАВНИВАНИИ (v1.346) ═══
       Запрос пользователя: «у П2 прижми номера к паттернам в любом выравнивании».
       КАК БЫЛО. В ячейке П2 печаталось «номер + отступ + паттерн», то есть номер стоял ПЕРЕД
       отступом — у левого края ячейки, а отступ выравнивания разводил его с паттерном тем шире,
       чем правее стоял сам паттерн. Прилипшим номер выглядел только при выравнивании влево, где
       отступа нет вовсе. У П1 такой беды не было: там номер печатается ПОСЛЕ паттерна, вплотную.
       КАК СТАЛО. Порядок «отступ + номер + паттерн»: номер входит внутрь отступа и оказывается
       вплотную слева от паттерна, куда бы выравнивание тот ни поставило.
       ОТСТУП УКОРАЧИВАЕМ НА ШИРИНУ НОМЕРА. Иначе пара «номер+паттерн» стала бы на numColsR
       столбцов шире и уехала бы вправо, то есть выравнивание колонки сбилось бы ровно на номер.
       Съев эти столбцы из отступа, мы оставляем ПРАВЫЙ край пары там же, где стоял правый край
       одного паттерна, — картинка не двигается, номер просто занимает место слева от него.
       Отступа не хватило (выравнивание влево, отступ нулевой) — номер честно сдвигает паттерн
       вправо на свою ширину: прижатым он остаётся в любом случае, а это и просили.
       Ширину берём ту же --num-w, что и у П1 (numColsL выше): одна на все строки, иначе строки с
       номерами разной длины разъехались бы по колонке. */
    const numColsR = (st.patNumR !== false && colStepPx > 0)
      ? Math.round((parseFloat(rootCS.getPropertyValue("--num-slot")) || 0) / colStepPx) : 0;
    const shR = patFieldShiftFor(pat2Align, patLen, i, "R", maxPatLen);
    const padR = shR > numColsR ? "&nbsp;".repeat(shR - numColsR) : "";
    ...
    if (p && p.text){
      ...
      patRight = '<span class="' + c2 + '"' + allHitStyle + '><span class="pat-shift"' + halfAttr(halfR, 0) + '>' + padR + numRightHtml + patTxtHtml + '</span>' + stepHtml + '</span>';
    } else {
      /* Пустая строка раскладывается ТОЧНО ТАК ЖЕ, как строка с паттерном (v1.322):
         те же padL/padR, тот же полушаг и та же поправка на переполнение, просто без
         самого текста. Порядок внутри сохранён: у П1 номер ПОСЛЕ отступа, у П2 ДО него. */
      pat = '<span class="pat"><span class="pat-shift"' + halfAttr(halfL, overL) + '>' + padL + numLeftHtml + '</span></span>';
      patRight = '<span class="pat2"><span class="pat-shift"' + halfAttr(halfR, 0) + '>' + padR + numRightHtml + '</span></span>';
    }
    ...
    for (let k = 0; k < s.length; k++) {
      ...
      if (lcsRow && lcsRow[k] && (bit === '0' || bit === '1')) {
        ...
        /* ЯРКОСТЬ ОТДЕЛЬНО ОТ ЦВЕТА (v1.506, запрос пользователя: «пусть тот символ, что на
           следующем шаге перенесётся, приглуши яркость ему»). Класс .wrap-bit-next только гасит —
           краску держит .wrap-bit-left/.wrap-bit-right, и по ней по-прежнему видно, чья это ручка.
           До этого «уже ушедшее» и «уйдёт следующим» горели одинаково, и край остатка было не
           отличить от края куска. */
        const sideCls = wrapHot ? ' wrap-bit-left' : ' wrap-bit-right';
        const sideNm = wrapHot ? 'ЛЕВАЯ ручка (синий)' : 'ПРАВАЯ ручка (красный)';
        const tip = wrapDim
          ? sideNm + ': ЭТОТ БИТ УЙДЁТ СЛЕДУЮЩИМ ШАГОМ — потому и приглушён. Всё, что она уже перенесла, стоит в строке-куске полным цветом'
          : sideNm + ': уже перенесено этой ручкой. Приглушённый бит того же цвета — тот, что уйдёт следующим шагом';
        emit('<span class="b' + bit + sideCls + (wrapDim ? ' wrap-bit-next' : '') + '" title="' + tip + '"' +
          colAttr + '>', bit, mrg);
        ...
        /* ═══ ФОН РОВНО В ВЫСОТУ СТРОКИ (испр. v1.400 → v1.401) ═══
           Баг-репорт со скриншотом: «не поменялось» — подсветка «🌈 Все паттерны» по-прежнему
           налезала на соседние строки.
           В v1.400 я подрезал по высоте все фоны бит, какие нашёл в стилях, — а этот задаётся
           ИНЛАЙНОМ, прямо здесь, и в стилях его нет вовсе. Правило background-size из CSS ему тоже
           не помогло бы: оно не режет фон, заданный цветом, а только картинку.
           Поэтому тот же приём применяем на месте: цвет заворачиваем в градиент из него самого в
           себя же и ограничиваем высоту. Цвет остаётся ровно тем, что вернула allPatColor, —
           меняется только высота пятна.
           --row-h читается из CSS в момент отрисовки, отдельной величины тут не заводим: строки
           живут той же переменной. */
        const allPatBg = allPatColor(allPatBit);
        emit('<span class="b' + bit + ' allpat-bit" style="background:linear-gradient(' +
             allPatBg + ',' + allPatBg + ') no-repeat center / 100% var(--row-h, 12px)' +
    ...
    /* ВСЕ НАЛОЖЕНИЯ ЭТОЙ СТРОКИ (список — v1.124). Рисуются по порядку списка, поэтому №2 ложится
       поверх №1, а №3 поверх обоих — тем же порядком, каким их накладывает расчёт
       (mergePasteIntoRow в fold-1-core.js). Индекс блока едет в data-pi: по нему обработчик мыши
    ...
    for (let pi = 0; pi < pasteAll.length && !belowLine; pi++) {
      ...
      const pk = i - pb.row;
      const prow = (pk >= 0 && pk < pb.rows.length) ? pb.rows[pk] : null;
      if (prow && prow.txt) {
        /* ПОЛУШАГ ИСХОДНОЙ СТРОКИ (prow.hf, в ПОЛУСТОЛБЦАХ, v1.107, запрос пользователя:
           "выравнивание надо тоже брать что и у оригинала"). Целую часть сдвига блок носит в off
           (её снимает rowShiftFor при копировании), но у «½»-выравниваний — «Центр ½», «Лесенка ½»,
           «Ось 1.2», «ОсьБит ½» — строка стоит ещё и на ПОЛСИМВОЛА в сторону (см. extraCh выше и
           rowHalf2x в fold-1-core.js), а целочисленный off такую поправку выразить не может. Без неё
           копия, снятая с «½»-раскладки, ложилась в целые столбцы и переставала совпадать с
           оригиналом. Здесь эта поправка — чисто визуальная, ровно как у самих строк: в расчёт
           (mergePasteIntoRow) наложение по-прежнему входит целыми столбцами. */
        const pasteCols = pb.col + (prow.off || 0) + (st.axisCenterOffset || 0) + (prow.hf || 0) / 2 + (pb.half || 0) / 2;
        /* ═══ ОТСЧЁТ ОТ СОДЕРЖИМОГО СТРОКИ, А НЕ ОТ КРАЯ ПОЛЯ (испр. v1.122) ═══
           Баг-репорт пользователя: «подсветка совсем не там», «биты вообще не касаются друг друга
           в разных местах». Причина не в маске и не в полушагах, а в том, от чего отсчитывался сам
           блок. .bits — ФЛЕКС-КОНТЕЙНЕР, и при «по центру» (.al-center, .al-halfcenter) содержимое
           строки в нём ЦЕНТРИРУЕТСЯ: его левый край стоит в 50% минус половина ширины содержимого.
           А блок — position:absolute с left от ЛЕВОГО КРАЯ .bits, и центрирования не получал вовсе.
           Строка и блок жили в разных началах отсчёта, и разъезжались они ровно на половину
           свободного места в поле — то есть на десятки столбцов, а вовсе не на полстолбца, которые
           я ловил в v1.119.
           Ширину содержимого знаем точно: это renderWidth столбцов (строка печатается с
           добивкой пустотой слева и справа ровно до неё, см. blankRun/padRight выше). Поэтому
           центрирование выражается прямо в CSS, без единого замера в пикселях:
             по центру — от 50% минус половина содержимого;
             вправо   — от 100% минус вся ширина содержимого;
             влево и все прочие режимы — от левого края, как и было.
           После этой правки paste.col наконец означает СТОЛБЕЦ СТРОКИ, а не абстрактное смещение
           в поле: и расчёт (mergePasteIntoRow), и подсветка, и картинка говорят об одном и том же.
           ВНИМАНИЕ при чтении старого кэша: блок, поставленный до 1.122, был сохранён в прежних
           координатах и на центрированной цепочке один раз прыгнет на своё настоящее место. */
        const contentCols = renderWidth;
        const leftCalc = (px, anchor) => (px < 0
          ? "calc(" + anchor + " - " + Math.abs(px).toFixed(2) + "px)"
          : "calc(" + anchor + " + " + px.toFixed(2) + "px)");
        const isCenterAl = (st.align === "center" || st.align === "halfcenter");
        const isRightAl = (st.align === "right");
        /* Одна формула на все накладки блока (v1.133): ручка, крестик и сами цифры обязаны
           отсчитываться одинаково, иначе крестик уедет от ручки при центрированной цепочке. */
        const leftAt = (cols) => isCenterAl
          ? leftCalc((cols - contentCols / 2) * colStepPx, "50%")
          : (isRightAl
              ? leftCalc((cols - contentCols) * colStepPx, "100%")
              : (cols * colStepPx).toFixed(2) + "px");
        const leftPx = leftAt(pasteCols);
        const offCls = (pb.on === false ? " paste-off" : "");
        /* ЧТО ПЕЧАТАЕМ (v1.115; координаты исправлены в v1.117 по баг-репорту «подсветка совсем не
           там»). При «Поверх» — сами цифры куска, при XOR/AND — РЕЗУЛЬТАТ операции с битами строки
           под блоком, посчитанный той же pasteCombine, которой считает и расчёт. Вторая строка
           (chg) помечает позиции, где результат отличается от бита строки: их красит .ps-chg.
           ОТДАЁМ ИМЕННО ТО, ЧЕМ САМИ РИСУЕМ: строку s (сырая st.rows[i] с прореживанием) и её
           начало shiftPad — те же значения, по которым выше напечатаны биты этой строки. Раньше
           функция брала строку сама, в координатах расчёта (mirroredRowBits + rowShiftFor), и с
           включёнными зеркалами подсветка уезжала на ширину места под них (mirrorPadL).
           st.axisCenterOffset тут не нужен: он двигает и строку, и блок, и во взаимном смещении
           сокращается — потому в leftPx он есть, а здесь его нет. */
        /* ПОЛУШАГИ ОБЕИХ СТОРОН (испр. v1.119). rowHalf — собственная поправка ЭТОЙ строки: всё,
           что render добавил ей трансформом сверх общего axisCenterOffset (полшага «½»-режимов по
           чётности maxLen−длина, поправки осей, отрицательный сдвиг). blockHalf — полушаг блока:
           снятый с исходной строки prow.hf плюс тот, что накрутили стрелками (paste.half).
           Общий axisCenterOffset не вычитаем и не прибавляем — он двигает строку и блок одинаково
           и во взаимном смещении сокращается. */
        ...
        /* ЗАМЕР ДЛЯ ДИАГНОСТИКИ (v1.118—1.121) УБРАН: координаты сошлись в v1.122, подсказка
           вернулась к обычному тексту. data-pi — номер блока в списке: по нему обработчик мыши
           понимает, за какой из блоков схватились (см. mousedown в fold-4-tools.js). */
  ...
  /* Третий аргумент (v1.337) — ширина ПОСТОЯННОГО слота колонки номеров: те же два кандидата, что
     и у numPadW, но всегда в двоичном виде, то есть в самом широком из режимов. Просвет слева от
     бит держится по нему и не меняется при переключении «№10 / №01 / №—» — иначе на каждое
     нажатие вся левая сторона уезжала вбок (весь разбор — у fitNumW в fold-5-ui.js). */
  const numSlotPadW = Math.max(
  ...
  /* ═══ ПРОСВЕТ ПОД НОМЕРА — СРАЗУ ЗА ИХ ЗАМЕРОМ (v1.420) ═══
     ...
     Поправка на переполнение (ниже в этой же функции) сравнивает содержимое ячейки П1 с её ВИДИМОЙ
     шириной, а видимая = --pat-w минус просвет под номера. Пока просвет считался только в
     updateSplitPositions — то есть ПОСЛЕ раскладки строк, — поправка брала прошлое число и
     отставала на кадр: номера уже шире, а биты П1 ещё не сдвинуты.
     Зовём здесь: fitNumW строкой выше только что измерила ширину номера, значит просвет считается
     по свежей величине и попадает в ту же отрисовку, что и сами строки. */
  ...
  // Общий сдвиг крайних полей (v0.976) — в пиксели его переводит шаг столбца, а тот меряется по
  // уже отрисованным битам, поэтому только здесь, после вставки строк.
...
/* ИДЁТ ЛИ СЕЙЧАС АВТО-ПРОГОН — обоих видов сразу: общее "▶ Авто" (st.running) и своё "Авто" у
   Паттерн-цепочки (patChainAutoTimer, живёт в fold-4-tools.js, загружается ПОСЛЕ этого файла —
   отсюда typeof-проверка, тот же приём, что и у остальных forward-ссылок между fold-*.js). */
function isAutoRunning(){
  return !!(st.running || (typeof patChainAutoTimer !== "undefined" && patChainAutoTimer));
}
/* Уведомление скрывает ЛЮБОЙ клик где угодно (v0.989, запрос пользователя: "пусть уведомления не
   исчезают пока не кликнуть где-нибудь, при неавтоматических режимах работы") — вешаем один раз
   на document, capture:true, чтобы отработать раньше любых stopPropagation по дороге. Хватать
   специально клик именно ПО плашке не нужно — она и так пропадает, лишний код не даёт другого
   поведения. */
let msgClickBound = false;
function ensureMsgClickDismiss(){
  if (msgClickBound) return;
  msgClickBound = true;
  document.addEventListener("click", e => {
    // Клик ПО САМОЙ ПЛАШКЕ — не про закрытие, а про перестановку в угол (v1.002, см.
    // applyMsgPos() ниже) — тот обработчик сам решает её судьбу, сюда не лезем.
...
/* КЛИК ПО ПЛАШКЕ — ПО КРУГУ МЕЖДУ НИЗОМ-ЦЕНТРОМ И ЧЕТЫРЬМЯ УГЛАМИ (v1.002, запрос пользователя:
   ...
   кругового обхода — по часовой от низа: центр → низ-справа → низ-слева → верх-справа →
   верх-слева → снова центр. Выбор пишется в msgPos (var, fold-1-core.js) и в кэш через
   captureUiSettings/applyUiSettings (fold-5-ui.js) — как и у прочих настроек вида, переживает
   перезагрузку. saveCacheSoon, а не saveCache — клик может повторяться быстро при переборе угла,
   как и любой другой ползунок/тумблер вида. */
const MSG_POS_ORDER = ["", "msg-pos-br", "msg-pos-bl", "msg-pos-tr", "msg-pos-tl"];
function applyMsgPos(pos){
  const el = document.getElementById("msg");
  msgPos = MSG_POS_ORDER.includes(pos) ? pos : "";
  if (!el) return;
  MSG_POS_ORDER.forEach(c => { if (c) el.classList.remove(c); });
  if (msgPos) el.classList.add(msgPos);
}
/* ДЕЛЕГИРОВАНИЕ ЧЕРЕЗ document, А НЕ ПРЯМОЕ addEventListener НА #msg (испр. v1.003, баг
   пользователя: "не ставит" — переключение по клику молчало вообще). Причина: теги <script> в
   fold.html идут ДО <div id="msg">, поэтому в момент, когда этот код выполнялся при загрузке
   страницы (обычный синхронный <script>, выполняется сразу по ходу парсинга HTML), самого #msg
   в DOM ещё не было — document.getElementById("msg") находил null, и addEventListener тихо
   не подключался вовсе. say()/ensureMsgClickDismiss() эту же ловушку не задевают, потому что
   ищут #msg ВНУТРИ своих функций — те выполняются позже, по вызову, когда страница уже вся
   разобрана. Здесь — тот же приём: слушаем document (он есть всегда), а сам #msg ищем только
   в момент клика. */
document.addEventListener("click", e => {
  if (!(e.target.closest && e.target.closest("#msg"))) return;
  e.stopPropagation();
  const next = MSG_POS_ORDER[(MSG_POS_ORDER.indexOf(msgPos) + 1) % MSG_POS_ORDER.length];
  applyMsgPos(next);
  if (typeof saveCacheSoon === "function") saveCacheSoon();
});
function say(t, kind){
  const el = document.getElementById("msg");
  if (!el) return;
  ...
  /* ═══ ТЕКСТ В СВОЁМ ЯЩИКЕ (v1.359) ═══
     Запрос пользователя: «фон прозрачный, а фон на тексте, символах — от фона поля цепочек».
     Плашка стала прозрачной (см. #msg в стилях), и подложка нужна не ей, а самим буквам — иначе
     сообщение читается поверх бит. Фон нельзя повесить на #msg: он закрасил бы весь прямоугольник,
     включая пустое место под прижатым кверху текстом. Значит нужен элемент ровно по тексту, а его
     до сих пор не было — строка ставилась прямо в textContent.
     Заводим span. textContent у него, а не innerHTML у плашки: сообщения собираются из чего угодно,
     включая имена паттернов и подписи кнопок, и разметке там взяться неоткуда — пусть и не берётся.
     Плашку сначала опустошаем, потом вкладываем span: replaceChildren сделал бы то же самое, но
     этот путь короче и не зависит от его поддержки. */
  el.textContent = "";
  const msgTxtEl = document.createElement("span");
  msgTxtEl.className = "msg-txt";
  msgTxtEl.textContent = t;
  el.appendChild(msgTxtEl);
  ...
  /* ВНЕ АВТО-ПРОГОНА — ПЛАШКА ВИСИТ, ПОКА НЕ КЛИКНУТ (v0.989). Автотаймер 2500мс остаётся ТОЛЬКО
     во время "▶ Авто"/Паттерн-цепочки-Авто: там сообщений много и подряд, ждать клика на каждое
     означало бы стоящий прогон (та же причина, по которой плашка вообще умеет расти — v0.962,
     см. комментарий у msgBoxHeight выше). В остальное время — а это подавляющее большинство
     сообщений в приложении — плашка держится, пока не кликнут где угодно. */
  if (isAutoRunning()) {
    msgTimer = setTimeout(() => {
      el.classList.remove("show");
    }, 2500);
  } else {
    ensureMsgClickDismiss();
  }
}
...
if (canvasEl) {
  ...
  /* Ctrl+V НАД КАНВАСОМ — ОТКЛЮЧЕНО (v0.998, запрос пользователя: "Ctrl+V в браузере вставляет
     текст в поля паттернов" + "вообще убери контроль V", подтверждено ещё раз после того, как
     первая попытка починки попала не в ту вставку — "пишет лог [...] остаток баг": настоящий
     виновник был здесь, а не в текстовом поле списка масок (см. v0.997)).
     БЫЛО (v0.9xx): тот же приём, что drag&drop файла — Ctrl+V, пока курсор над канвасом,
     ПОЛНОСТЬЮ заменял tplRows/tplPats содержимым буфера обмена, определяя строки/паттерны по
     последней известной позиции мыши. НИКАКОГО snapshot() перед заменой не было — Undo это не
     видел вообще, и случайный Ctrl+V (например, обычная попытка вставить текст куда-то ещё, а
     курсор в этот момент просто был над полотном) молча и безвозвратно стирал цепочку.
     Чинить его вместо этого (снять снимок перед заменой) не стали — попросили просто убрать.
     Drag&drop файла (см. .addEventListener("drop", ...) выше) не тронут — это отдельный жест,
     недвусмысленный (нужно явно перетащить файл), и жалоб на него не было. */
}
...
function exportAllTabs(){
  ...
  /* ═══ В ФАЙЛ УХОДИТ ВСЁ, ЧТО ПРИЛОЖЕНИЕ ПОМНИТ (v1.453) ═══
     Вопрос пользователя: «и они работают с новыми запоминаниями?» — нет, не работали.
     Выгружались одни вкладки, а всё ОБЩЕЕ (одно на все цепочки) оставалось в браузере: настройки
     вида и поиска, снимок настроек и раскладки панелей от кнопки «Сохр. настройки», кэш паттернов
     и свои слоты цветов. Перенести работу на другую машину файл поэтому не мог: цепочки приезжали,
     а вид приходилось настраивать заново.
     Кладём в файл ровно то же, что лежит в кэше браузера (см. saveCache в fold-5-ui.js), плюс
     заголовок. Совместимость в обе стороны: в старом файле новых полей просто нет, и импорт их не
     ждёт; новый файл старой версией читается как прежде — она берёт только tabs/activeTab. */
  const payload = {
    app: "Zerkalius Fold", exportedAt: new Date().toISOString(),
    tabs: st.tabs, activeTab: st.activeTab,
    ui: (typeof captureUiSettings === "function") ? captureUiSettings() : null,
...
function clearAllCache(){
  const msg = "Полностью очистить кэш?\n\n" +
    ...
    "Если цепочки нужны — сначала выгрузи их кнопкой «⬇ Экспорт».\n\nПродолжить?";
...
if (importTabsInputEl) {
  importTabsInputEl.onchange = () => {
    ...
    reader.onload = () => {
      ...
      saveCache();
      /* ═══ ОБЩЕЕ ИЗ ФАЙЛА — ТОЖЕ (v1.453) ═══
         Всё, что не принадлежит отдельной вкладке: вид и поиск, снимок с раскладкой панелей, кэш
         паттернов, слоты цветов. Каждое поле применяется, только если оно в файле есть, — старые
         выгрузки (одни вкладки) читаются как и раньше.
         Раскладку кладём тем же путём, что и сброс настроек: applyLayout плюс setOverlayHidden на
         каждое окно, иначе видимость останется в переменной и на экран не попадёт. */
      const extras = [];
      if (!Array.isArray(data)) {
        if (Array.isArray(data.patBank)) { st.patBank = data.patBank; extras.push("кэш паттернов"); }
        if (Array.isArray(data.colorSlots)) { st.colorSlots = data.colorSlots; extras.push("слоты цветов"); }
        if (data.savedUiSettings) st.savedUiSettings = data.savedUiSettings;
        if (data.savedLayout) st.savedLayout = data.savedLayout;
        if (data.ui && typeof applyUiSettings === "function") { applyUiSettings(data.ui); extras.push("настройки вида/поиска"); }
        if (data.savedLayout && typeof applyLayout === "function") {
          applyLayout(data.savedLayout);
          ...
          extras.push("раскладка панелей");
        }
      }
      render();
      if (typeof saveLayout === "function") saveLayout();
      say(`Загружено из файла: ${tabs.length} вкладок` + (extras.length ? ", а также " + extras.join(", ") + "." : "."));
    };
