...
function renderTabs() {
  ...
  '<div class="chain-dd-footer">' +
    ...
    '<button type="button" class="chain-dd-file" id="bDdExport" data-act="export" title="Сохранить ВСЕ вкладки-цепочки (со всеми их строками/паттернами/настройками) в файл .json">⬇ Файл</button>' +
    '<button type="button" class="chain-dd-file" id="bDdImport" data-act="import" title="Загрузить вкладки-цепочки из файла .json (заменит текущие все)">⬆ Файл</button>' +
    '<button type="button" class="chain-dd-file chain-dd-wipe" id="bDdWipe" data-act="wipe" title="ПОЛНОСТЬЮ очистить кэш приложения в браузере: все вкладки-цепочки, настройки и раскладку панелей. Нужно, когда сохранённое состояние само вешает вкладку. Сначала выгрузи нужное кнопкой «⬇ Файл»">🗑 Кэш</button>' +
    '</div>' +
  ...
  // (см. copySelectedRows/clipRowsHtml).
    ...
    '<button type="button" class="chain-dd-file chain-dd-ui" id="bDdUiReset" data-act="uireset" title="Сбросить настройки вида/поиска к значениям по умолчанию — данные цепочек не трогает">↺ Сброс настроек</button>' +
    '</div>' +
  '</div>';
}
...
function render(){
  ...
  for (let i = vr.lo; i <= vr.hi; i++){
    ...
    const colPx = Math.max(0,
      ...
      - (st.patNumL ? (parseFloat(rootCS.getPropertyValue("--num-slot")) || 0) : 0));
